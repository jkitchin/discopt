...
| Decomposition Advisor             | Planned (design) | Automatic decomposition-as-a-transformation pass: analyze model structure, discover exploitable blocks, score/rank candidate decompositions, explain the recommendation, and auto-reformulate so `solve()` picks no-decomp / Benders / GBD / Lagrangian / Dantzig–Wolfe / ADMM / Schur / progressive hedging without manual `decomposition="..."`. Extends the existing `DecompositionStructure`/`detect_decomposition` layer; the shipped Benders/GBD/Lagrangian drivers become plugins behind a common reformulation IR, and the deferred Dantzig–Wolfe slots in as one more method. Design spec: `docs/design/decomposition-advisor.md` |
