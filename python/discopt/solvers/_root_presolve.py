...
def tighten_bigm_coefficients(
    ...
    *,
    ...
    The transformation removes **no** integer-feasible point (it only shrinks
    the LP relaxation), so it is sound. It mutates ``model._constraints`` in
    place. Returns the total number of rows whose coefficients were tightened
    across all rounds (0 when the flag is off or nothing tightens).
    ...
    def is_binary(i: int, lb: float, ub: float) -> bool:
        vt = str(getattr(blocks[i], "vtype", getattr(blocks[i], "var_type", "")))
        return "IN" in vt.upper() and lb == 0.0 and ub == 1.0
    ...
    for _round in range(max_rounds):
        ...
        for con in model._constraints:
            ...
            if not row_changed:
                continue
            # Reflect back to the original sense and rewrite the row in place.
            a_orig = a * sgn
            rhs_orig = rhs * sgn + const
            con.body = _rebuild_linear_body(blocks, a_orig, const)
            con.rhs = float(rhs_orig)
            round_changed += 1
