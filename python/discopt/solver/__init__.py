...
# Card 4b: the native spatial kernel now lives in ``discopt.solver.native_kernel``.
# Re-exported here are (a) the names this package calls and (b) the read-only
# helpers the test suite reaches for through ``discopt.solver``; the explicit
# ``x as x`` form marks them as deliberate re-exports rather than unused imports.
#
# The two functions the tests MONKEYPATCH -- ``_native_kernel_seed`` and
# ``_native_kernel_seed_candidates`` -- are deliberately NOT re-exported. A
# re-export would let ``monkeypatch.setattr(discopt.solver, ...)`` succeed while
# the real call site inside ``native_kernel`` kept using the original function: a
# patch that silently does nothing, and in one case (``must_not_run``) a negative
# assertion that would then pass vacuously. Without the re-export the stale patch
# raises ``AttributeError`` instead (CLAUDE.md §6).
from discopt.solver.native_kernel import _native_kernel_feature_safe as _native_kernel_feature_safe
from discopt.solver.native_kernel import _native_kernel_verify_point as _native_kernel_verify_point
from discopt.solver.native_kernel import _native_nlp_enabled as _native_nlp_enabled
from discopt.solver.native_kernel import _native_seed_bracket as _native_seed_bracket
from discopt.solver.native_kernel import (
