...
# Named functions for which this compiler can build a rigorous convex/concave
# relaxation -- i.e. those eligible for the *certified-global* spatial B&B path.
# Keep in sync with the dispatch in ``_compile_relax_node`` (the ``_univariate_relax``
# and ``_envelope_relax`` tables plus the special-cased ``sign``/``min``/``max``).
# A FunctionCall whose name is absent here forces discopt onto the local
# (uncertified) path. Binary operators (+, -, *, /, **) are always relaxable.
GLOBALLY_RELAXABLE_FUNCS: frozenset[str] = frozenset(
    {
        # univariate McCormick rules
        "exp",
        "log",
        "log2",
        "log10",
        ...
        "abs",
        "entropy",
        # envelope-based univariate rules
        "asinh",
        "acosh",
        "atanh",
        "erf",
...
def nonrelaxable_functions(model: Model) -> set[str]:
    """Return the set of function names in *model* with no rigorous relaxation.

    These force the solver onto the local (uncertified) path rather than the
    certified-global spatial branch-and-bound. An empty set means every nonlinear
    function in the model is globally relaxable.
    """
    found: set[str] = set()

    def walk(node: object) -> None:
        if isinstance(node, FunctionCall):
            if node.func_name not in GLOBALLY_RELAXABLE_FUNCS:
                found.add(node.func_name)
            for a in node.args:
                walk(a)
        elif isinstance(node, CustomCall):
            found.add(str(node.name))  # opaque AD-only node: never globally relaxable
            for a in node.args:
                walk(a)
            ...
            # MatMulExpression and any other composite: introspect children.
