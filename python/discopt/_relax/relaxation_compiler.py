...
if TYPE_CHECKING:
    from discopt._relax.learned_relaxations import LearnedRelaxationRegistry
...
from discopt._relax.multivariate_mccormick import get_composition_rule
...
def _try_extract_signed_abs_product(expr: Expression, model: Model) -> int | None:
    ...
    (:func:`discopt._relax.symbolic.domains.gas.weymouth_relax`) instead of the
...
def _try_extract_signed_power(expr: Expression, model: Model) -> tuple[int, float] | None:
    ...
    :func:`discopt._relax.symbolic.domains.gas.signed_power_relax`, which is a tight
...
def _try_extract_xlogx(expr: Expression, model: Model) -> int | None:
    ...
    ``x ln x`` (:func:`discopt._relax.symbolic.domains.chemeng.xlogx_relax`) — tighter
    ...
    if isinstance(expr, BinaryOp):
        ...
        if op == "*":
            ...
            if gas_off is not None:
                try:
                    from discopt._relax.symbolic.domains.gas import weymouth_relax
                except ImportError:
            ...
            if xlogx_off is not None:
                try:
                    from discopt._relax.symbolic.domains.chemeng import xlogx_relax
                except ImportError:
            ...
            if tri_offsets is not None:
                if tuning.trilinear_exact:
                    from discopt._relax.envelopes import relax_trilinear_exact as _relax_trilinear
                else:
            ...
            if sig_factors is not None:
                from discopt._relax.envelopes import relax_signomial_multi
        ...
        if op == "/":
            ...
            if monod is not None:
                _mo_off, _mo_k = monod
                try:
                    from discopt._relax.symbolic.domains.chemeng import saturating_relax
                except ImportError:
    ...
    if isinstance(expr, FunctionCall):
        ...
        if name in ("sin", "cos") and len(expr.args) == 1:
            ...
            if isinstance(arg, (Variable, IndexExpression)):
                from discopt._relax.envelopes import relax_cos_tight, relax_sin_tight
        ...
        if name == "exp" and len(expr.args) == 1:
            ...
            if arr is not None:
                _ar_off, _ar_c = arr
                try:
                    from discopt._relax.symbolic.domains.chemeng import arrhenius_relax
                except ImportError:
        ...
        if name in _univariate_relax:
            ...
            # python/discopt/_relax/multivariate_mccormick.py and issue #51 (M1).
        ...
        if name == "min":
            ...
            def fn(x_cv, x_cc, lb, ub):
                ...
                from discopt._relax.mccormick import relax_min
        ...
        if name == "max":
            ...
            def fn(x_cv, x_cc, lb, ub):
                cv_a, cc_a = a_fn(x_cv, x_cc, lb, ub)
                cv_b, cc_b = b_fn(x_cv, x_cc, lb, ub)
                from discopt._relax.mccormick import relax_max
...
def compile_relaxation(
    ...
    if arithmetic == "alphabb":
        from discopt._relax.alphabb import compile_alphabb_relaxation
