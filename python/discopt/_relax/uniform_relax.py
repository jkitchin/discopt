...
class _ModelAnalysisCache:
    ...
    def __init__(self, model: Model, dag: CanonicalDAG):
        from discopt._relax.canonical_expr import _flat_var_expr
...
class _Builder:
    ...
    def _curvature_cert(
        ...
        from discopt._relax.milp_relaxation import _multivar_box_curvature
...
def _emit_logspace_band(ctx: _Builder, w: int, node: CNode) -> bool:
    ...
    * ``w = coef·exp(s)`` — convex (``coef>0``) / concave (``coef<0``), exact
    ...
    """
    from discopt._relax.milp_relaxation import _extract_positive_product
...
def _emit_quadratic_rlt(
    ...
    """
    from discopt._relax.milp_relaxation import _quadratic_constraint_forms
    from discopt._relax.rlt_cuts import rlt_quadratic_bound_cut_row
