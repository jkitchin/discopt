...
``__getattr__``: importing a JAX-free submodule such as ``discopt._relax.deadline``
...
``discopt._relax.compile_expression`` still works unchanged.
"""
...
def __getattr__(name: str) -> object:
    if name in __all__:
        from discopt._relax import dag_compiler
