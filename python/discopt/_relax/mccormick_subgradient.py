...
:mod:`discopt._relax.mcbox`. Where the original walked the model AST into hand-rolled
``(cv, cc)`` closures (affine / QP scope, ``jax.grad`` subgradients), this interprets
the AST into :class:`~discopt._relax.mcbox.MCBox` operations — so the relaxation and its
...
from discopt._relax import mcbox as _mcbox
from discopt._relax.mcbox import UnsupportedMcboxOp, mcbox_leaves
from discopt._relax.relaxation_compiler import _resolve_scalar_var_offset
    ...
    for r in range(1, max_rounds + 1):
        ...
        if backend == "simplex":
            try:
                ...
                from discopt._relax.mccormick_lp import _append_relax_rows
