...
The guard shipped once on its own and was reverted: it broke eight tests that
passed on main, all writing ``m.minimize(0 * field[i])`` to mean "no objective,
just find a feasible trajectory". On a 2-D ``MOLBuilder`` field that indexes one
axis and leaves a whole spatial row. Measuring the idiom showed it does not work
cleanly either -- it prints

    objective reconciliation skipped (TypeError: cannot reshape array of shape
    (5,) (size 5) into shape (1,) (size 1)); leaving objective=0.0 as the solver
    reported it.

so a non-scalar objective silently turns the reconciliation check off, and the
reported value is right only because the objective is identically zero. The
guard therefore ships WITH the migration of those call sites, and
``TestTheNoObjectiveIdiomHasAScalarSpelling`` pins that the migration is a no-op
on the answer.
"""
...
class TestTheNoObjectiveIdiomHasAScalarSpelling:
    """ "No objective, just find a feasible point" must survive the guard.

    Every spelling below is scalar and reconciles; the 2-D-field one the guard
    refuses is the only one that did not. Measured on the heat-equation model
    from ``test_dae_fd_mol.py``, all three give a **bit-identical** trajectory
    (``max|diff| = 0.0``), so migrating the call sites changed no answer.
    """

    @staticmethod
    def _heat_model():
        from discopt.dae import BoundaryCondition, ContinuousSet, MOLBuilder, SpatialSet
        ...
        mol.set_pde(lambda t, z, f, fz, fzz, c: {"u": 0.1 * fzz["u"]})
        mol.discretize()
        return m, mol.get_field("u")

    def test_a_2d_field_row_is_refused_and_says_what_to_do(self):
        m, u = self._heat_model()
        assert len(u.shape) == 2, u.shape
        with pytest.raises(ValueError, match="scalar"):
            m.minimize(0 * u[0])

    @pytest.mark.parametrize("spelling", ["zero_constant", "indexed_element"])
    def test_the_scalar_spellings_solve_to_the_same_trajectory(self, spelling):
        ref_m, ref_u = self._heat_model()
        ref_m.minimize(0 * ref_u[0, 0])
        ref = ref_m.solve()
        assert ref.status == "optimal", ref.status
        ref_traj = np.asarray(ref.x[ref_u.name], float)

        m, u = self._heat_model()
        m.minimize(0.0 if spelling == "zero_constant" else 0 * u[0, 0])
        r = m.solve()
        assert r.status == "optimal", r.status
        assert r.objective == pytest.approx(0.0, abs=1e-12)
        assert np.array_equal(np.asarray(r.x[u.name], float), ref_traj), (
            "the scalar spelling changed the trajectory"
        )

    def test_a_1d_state_element_was_never_affected(self):
        """An FDBuilder state is 1-D, so ``state[0]`` is already scalar."""
        from discopt.dae import ContinuousSet, FDBuilder

        m = dm.Model("fd")
        cs = ContinuousSet("t", bounds=(0, 2), nfe=20)
        fd = FDBuilder(m, cs, method="backward")
        fd.add_state("x", initial=1.0, bounds=(-5, 5))
        fd.set_ode(lambda t, s, a, c: {"x": -s["x"]})
        fd.discretize()
        state = fd.get_state("x")
        assert len(state.shape) == 1, state.shape
        m.minimize(0 * state[0])  # no raise
        assert m.solve(time_limit=60.0).status == "optimal"
