...
def _composite_square_model():
    """nvs09's per-variable composite ``(ln(x-2))**2 + (ln(10-x))**2`` over
    ``x in [3, 9]`` (issue #632, P1.1).

    The factorable AVM decomposes each ``(ln(x-2))**2`` into an inner atom
    ``w = ln(x-2)`` (relaxed by the exact concave log envelope) and an outer
    square ``s = w**2`` (a ``univariate_square`` column, relaxed by the convex
    square envelope over ``w``'s propagated range ``[0, ln 7]``). True minimum
    over the box is 3.66667 (at x=6, by fine scan / lever-a analysis).
    """
    m = discopt.Model("nvs09_composite")
    x = m.continuous("x", lb=3.0, ub=9.0)
    m.minimize(discopt.log(x - 2.0) ** 2 + discopt.log(10.0 - x) ** 2)
    return m


def _solve_composite_root_bound(model, square_separate: bool):
    prev = os.environ.get("DISCOPT_SQUARE_SEPARATE")
    os.environ["DISCOPT_SQUARE_SEPARATE"] = "1" if square_separate else "0"
    try:
        relaxer = mc.MccormickLPRelaxer(model)
        relaxer._inc = None  # exercise the separating cold path (see above)
        lb = np.array([3.0], dtype=np.float64)
        hi = np.array([9.0], dtype=np.float64)
        return relaxer.solve_at_node(lb, hi, time_limit=10.0)
    finally:
        if prev is None:
            os.environ.pop("DISCOPT_SQUARE_SEPARATE", None)
        else:
...
def test_composite_univariate_square_separation_recovers_avm_bound():
    """P1.1 (#632): a composite-univariate square ``f(x)**2`` must be tightened by
    the SAME LP-point tangent separator that covers bare-variable squares.

    ``_separate_univariate_square`` historically iterated only the ``monomial``
    column family, so the ``univariate_square`` columns produced by atomizing
    ``(ln(x-2))**2`` were left at their loose static envelope: the root bound
    collapsed to 0 even though atomization introduced the correct aux columns.
    The factorable AVM (``factorable_reform``) hid this only by rewriting the
    composite into a bare-variable monomial. Extending the separator to the
    composite columns recovers the composition bound directly.
    """
    m = _composite_square_model()

    off = _solve_composite_root_bound(m, square_separate=False)
    on = _solve_composite_root_bound(m, square_separate=True)

    assert off.status == "optimal" and off.lower_bound is not None
    assert on.status == "optimal" and on.lower_bound is not None

    # Without square separation the composite square is under-cut to ~0 (each
    # ``s = w**2`` can float to its column floor at the box interior).
    assert off.lower_bound <= 0.1, (
        f"static composite-square envelope unexpectedly tight ({off.lower_bound}); "
        f"the test no longer exercises the under-cut it guards"
    )
    # With separation the bound recovers the AVM composition value (~1.89328/var,
    # the 51.6% recovery measured in docs/dev/p1-atom-tightness-audit.md §3.1).
    assert on.lower_bound >= 1.89, (
        f"composite-square separation failed to recover the AVM bound: {on.lower_bound}"
    )
    # Soundness: a min-sense lower bound can never exceed the true optimum 3.66667.
    assert on.lower_bound <= 3.66667 + 1e-6, (
        f"composite-square bound {on.lower_bound} exceeds the true optimum 3.66667 "
        f"(an UNSOUND dual bound)"
    )
    assert off.lower_bound <= 3.66667 + 1e-6
    # And separation is strictly tighter than the static envelope.
    assert on.lower_bound > off.lower_bound + 1.0
