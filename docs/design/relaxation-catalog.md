...
| `min`/`max` over n terms | `xs.min()` / `xs.max()`, `dm.minimum(*terms)` | **balanced** fold of the binary nodes above | #1238. The relaxation stays binary *by measurement*: `uniform_relax._build_multivar` already emits the exact convex-hull facets `w ≥ a_i` / `w ≤ a_i` for `len(args) ≥ 2`, and a fold's intermediate auxes project straight out of that system — 300/300 fixed-box comparisons give the fold and an n-ary node the **same** bound (performance-plan §64). An n-ary *node* is not emitted: the value/interval consumers read `args[0], args[1]` and would drop the rest, which in FBBT is a wrong bound, not a loose one — the same reason `nl_parser.rs` folds `o11`/`o12` to binary (C-8). The balanced shape is for depth: `ceil(log2 n)` rather than `n-1`. |
