...
**Piecewise-linear functions** (`python/discopt/modeling/_piecewise.py`): `Model.piecewise(x, breakpoints, values)` declares a tabulated univariate function and lowers it, at declaration, to an exact MILP encoding — incremental {cite:p}`MarkowitzManne1957`, logarithmic {cite:p}`Vielma2011`, disaggregated convex combination, or a native SOS2 set {cite:p}`BealeTomlin1970` — following {cite:t}`Vielma2010`. Because the table *is* the function, the model solved is the model declared and the certificate carries over; an input whose domain escapes the breakpoint span is refused, not clamped. Its approximating counterpart, `dm.nonlinear_to_pwl(model)` (`python/discopt/modeling/_pwl_transform.py`), replaces univariate *nonlinear* terms by piecewise-linear constructs: `mode="outer"` builds a rigorous outer approximation (interval-arithmetic error bands with interval-AD mean-value tapers), so its bound is valid for the original model and a certificate requires a verified incumbent to meet it, with adaptive partition refinement {cite:p}`Nagarajan2019,Misener2011`; `mode="approximate"` never claims a bound. See {doc}`notebooks/piecewise_linear`.
