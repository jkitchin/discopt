...
| EX-7 ✅ RESOLVED | P2 | `_extract.py:153,287,475` | Array-valued `Constant` in a scalar position silently collapsed to its **first element** (`value.flat[0]`); `sum(constant_vector)` returned the first element rather than the sum [CONFIRMED — `sum([1,2,3,4])` extracted const `1.0` not `10.0`, in both the linear and quadratic extractors]. **FIXED (#413):** the three collapse sites are split by context. Reduction contexts (`SumExpression`/`SumOverExpression`) now sum the *full* constant array (new `_eval_constant_array` evaluates a constant sub-DAG preserving shape/broadcast, then `.sum()`), so `sum([1,2,3,4]) → 10.0` and `sum(2*[1,2,3]) → 12.0`. Genuinely scalar-required positions (bare additive `Constant`, `*`/`/` coefficients via `_get_constant_value`) route through `_scalar_constant`, which accepts a 0-d or size-1 array (broadcasts to scalar) and **raises loudly** on a multi-element array instead of dropping elements. Regression: `TestArrayConstantCollapse` (6 cases) in `python/tests/test_export.py`. |
