...
Status: **RESOLVED** (2026-07-14) — all PF0–PF5 items worked through the gates;
outcome synthesis in §4. Successor to `docs/dev/engine-performance-plan.md`
...
| PF5 incumbents + LP robustness | **SPIKE GO → FULL ITEM FAILED THE PF0 GATE → NOT LANDED** (`docs/dev/pf5-robustness-spike.md`) | **The time-limit bug is REAL** (warm-LP solve bindings hardcode `deadline: None`, so a warm re-solve can spin past the budget — contvar `solve_at_node(3s)` ran 24.9 s; and the PF0 "5 hangs" were partly jobs=4 CONTENTION, partly this). The spike prototyped the obvious fix — thread `time_limit → SimplexOptions.deadline` into the warm-LP path — and reported it bound-neutral. **The full-panel PF0 gate FALSIFIED that: proofs lost=5, 8 instances LOOSER (nvs05 2.70 vs 4.70, nvs09 −53.68 vs −43.70, m3 19.36 vs 37.8 [lost proof], nvs11 −2781 vs −431 [lost proof]).** Root cause: **OBBT's range-reduction probes RUN ON the warm-LP path**, so a blanket warm-LP deadline truncates the very probe LPs PF1 depends on → OBBT contracts boxes less → looser bounds. Confirmed by a controlled HEAD-vs-fix comparison (at HEAD without the fix those instances match baseline exactly: m3 optimal 37.8, nvs11 optimal −431; with the fix they collapse). The spike's "0 lost" was measured on too small a set. **REJECTED per §0.2 — a fix that regresses proofs/bounds does not land.** (This is the PF0 gate doing its job: the spike missed it, the panel caught it — cf. EP3.) C2 root-presolve, C3 incumbents, C4 contvar-conditioning all separately KILLed by the spike. **The time-limit bug remains OPEN as a follow-up** needing a SURGICAL fix (give OBBT probe LPs their own generous sub-budget, or cap only genuinely-runaway outer LP spins — not every warm-LP), tracked as a separate item; the blunt version is not viable. | NOT LANDED (gate failure recorded) |

## §4. Outcome synthesis (2026-07-14)

**One landed proof-mover, four gated non-lands — and the gates earned their keep.**

| Item | Verdict | Why it matters |
|---|---|---|
| PF0 | built | The panel + differential gate (LOOSER/CROSSED catcher, feasible-point sampler). Every verdict below is a gate result, not an opinion. |
| **PF1** | **LANDED** | FBBT range-reduction wired into the global spatial B&B node loop. **Vendored panel proved 36→41 (+5), hangs 5→1, wall −10 %**, sound (feasible-point 0 cuts). The one real cash-in. |
| PF2 | KILL | The "sound incremental node LP" (EP3-done-right) is **already shipped and sound** — differential green, nothing to build. |
| PF3 | KILL | Branching (hardcoded midpoint / reliability 8) is **inert** on the stuck set — not the binder. |
| PF4 | KILL (soundness) | No sound finite LMTD envelope exists (ε-pole inside the box); the hard gate **caught a would-be feasible-point-cutting bug** the spike had missed. Guard test pins it. |
| PF5 | REJECTED (gate) | The blunt warm-LP-deadline time-limit fix **regressed 5 proofs** (it truncates OBBT's probe LPs); the full-panel gate caught what the spike missed. |

**What this bought vs the EP series.** EP shipped 2 real wins out of 7 built items,
with 3 false premises discovered *after* building and 1 (EP3) reverted for an
unsound bound. PF built **1 item** (PF1) and spiked-then-killed the other four
*before* building — including PF4, where building blind would have shipped a
false-infeasible cut, and PF5, where it would have silently lost 5 proofs. The
spike-first + panel-gate cadence is strictly cheaper and strictly safer.

**The honest ceiling.** The remaining SOTA gap on the hard instances is *hard*,
not low-hanging: heatexch has an LMTD pole (relaxation correctly refuses a finite
bound); the stuck spatial trees are blocked by root strength + irreducible
per-node NLP/presolve cost, not by anything a simple lever removes. Closing them
needs genuinely new capability (pole-excluded sub-box LMTD relaxation deep in the
tree; a surgical time-limit budget that spares OBBT probes) — each a fresh spike
with its own entry experiment, not a mop-up.

**Open follow-ups (separate items, each needs its own spike + gate):**
1. **Time-limit honoring, done right** — cap only runaway outer LP spins / give
   OBBT probes their own sub-budget (the blunt PF5 version regressed proofs).
2. **Build-/root-presolve deadline** (hda/heatexch pre-node overrun) — bound-affecting,
   needs the differential gate (a naive truncation drops heatexch's dual bound).
3. **LMTD over pole-excluded sub-boxes** — deep-node relaxation, unmeasured value.
4. **contvar simplex conditioning** — scaling/restart, not a bumped iteration cap.

**Authoritative measurement is the maintainer's `global50`/`cert0` run** (the
...
| **F2′ — analytic IR gradient (root fix)** (`_separate_convex` grad path) | Both F2 symptoms (`eval_jaxpr` interpreter tax AND jit nondeterminism) stem from using JAX autodiff to differentiate a **known factorable expression**. `∇g` is the gradient of the engine's own `Expression` IR (closed-form atoms), computable analytically via `convexity/interval_ad.py` (forward-mode AD at a *point* box — the gradient interval collapses to the exact point gradient) with **no JAX on the hot path**. | **Bound-changing** (last bits differ from JAX grad) but **deterministic**. | **SPIKE → GO (gated, default-off)** `_compiled_analytic`, env `DISCOPT_ANALYTIC_SEPGRAD`, JAX fallback on any uncovered atom. **Entry:** point-grad matches `jax.grad` to ~1e-13 on all 7 lifted atom classes (exp/log/quadratic/`x/y`/sqrt/products/`(x+y)²/z`); ~1 ms/call vs eager-retrace jax 6–62 ms. **5-inst jobs=1 serial (2×/arm):** deterministic on ALL 5 (jit's tls2 flaw gone); matches eval on m3/nvs05/tspn05; **nvs09 gains a proof (optimal −43.134 vs eval timeout) and −9 s (22 vs 31 s)**. tls2's apparent diff is eval's OWN wall-budget flip (2.1↔2.449), not analytic. **Differential gate GREEN:** env-b at-least-as-tight per box (nvs09/tspn05/cvxnonsep roots identical), **feasible-point worst 1.82e-11 → SOUND, 0 cuts**. Unit test `test_f2prime_analytic_sepgrad.py` pins grad==autodiff + determinism. **FULL 62-inst jobs=1 panel:** analytic **proved 42 vs eval 41 (+1 nvs09, 0 lost)**, 0 infeasible, sound. The gate mechanically flagged one bound — `tls2` 2.1 vs 2.449 — but `tls2` is **wall-budget bimodal for eval itself** (measured eval draws: 2.1, 2.449, 2.1, … — 2.1 is the majority mode); analytic deterministically lands on eval's majority mode, so it is **within eval's own range, not below it** — a false-fail from diffing a deterministic arm against a nondeterministic reference's lucky draw (unlike jit, whose tls2 variance was jit's *own*). total_wall 900 vs 870 s (~+3 %: the point `interval_hessian` also builds the unused Hessian — a grad-only IR pass is the follow-up that recovers it). **Verdict: GO for default-on** — deterministic + sound + net +1 proof, strictly better than the nondeterministic `eval_jaxpr`; flip deferred to the maintainer's authoritative box. | gated prototype + test (this branch) |
