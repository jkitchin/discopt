...
| R1.2-G1 rigorous envelope (graduation prereq #1) | **DONE in-container (2026-07-12)** | Fixes review finding 2 at its root: `univariate_hull_envelope` is now **sound by construction between grid nodes**, not just at them. **Entry experiment (§4, GO):** a dense 5M-sample probe confirmed the old grid-sampled shift left real (tiny) violations — gauss_well `exp(-10000(x-0.05)²)` under=1.26e-6, nvs09 `(ln(x-2))²` under=1.1e-7 (>0 ⇒ unsound). The rigorous fix adds a proven **second-order remainder** `⅛·max(0,±f'')·h²` per side from a **sound interval Hessian** (`convexity.interval_ad.interval_hessian`, via new `_diag_hessian_enclosure`), where `h` is the fine-grid spacing; the correction is tiny (gauss_well 1.56e-6, ln_sq 2.46e-6 — far under the 1e-3·range usefulness gate) and after applying it the dense 5M violation goes **negative** (sound) for both. Kill-criterion (abstains on nvs09, or dense sample finds a violation) not triggered. **Global** interval-`f''` over a wide box is too loose (dependency blow-up: 3.6e8 for the Gaussian) so the remainder is taken over `_CURV_CELLS=512` cells and the *fine* `h²` — rigorous by enclosure monotonicity, tight because `h²` is minute. Unbounded enclosure (e.g. `tanh''` ∉ interval-AD grammar) ⇒ **refuse** (`None`), never ship an uncertified facet. Both flag-gated callers wired (composite + aliased `h**p`). **Verified in-container:** default path still byte-identical (contvar/hda/nvs09/tanksize == main, flag off); flag-ON corpus build 62/62 OK; `test_univariate_hull_lr2.py` rewritten to dense-adversarial (500k pts, tol 1e-7) through the real enclosure bridge — 11 passed incl. the sharp-Gaussian and cubic-inflection cases + `tanh` refusal; ruff/mypy clean. Still **flag-gated (default-off)** — graduation (default-ON) remains gated on prereqs #2 (full-corpus differential + feasible-point + `incorrect_count=0`) + #3 (nvs09/global50 cert) **on the local host**. |
