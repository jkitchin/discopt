...
| R1.2 univariate cutover (wiring) | **DONE — CP-1 (2026-07-12)** | `_collect_composite_univariate_relaxations` now consults the dispatch with the **1-D hull always-on** (no longer gated by `DISCOPT_UNIVARIATE_ENVELOPE`); the scattered `_should_claim_composite(allow_general=…)` + trig-table defers + `_pre_existing_claim` descent control collapse into the one dispatch decision. **In-container verification:** byte-identity diff = exactly **3 changed instances** (contvar, hda, nvs09), 59 identical (before/after fingerprints, same Rust binary); **nvs09 sound** (certified obj −43.13434 = optimum exactly, bound −51.26 ≤ optimum); smoke **645 passed**; adversarial **9 passed / 1 pre-existing fail** (`hda` — fails on clean tree too, not the wiring); offneutral byte-identity **61/61** (the graduated hull cancels on both sides; the still-flag-gated aliased-hull + H-LOG stay neutral); r12-boundary + auditor + shape-neutrality green (baseline re-generated for the 3 changed shapes). Perf: eliminated a double `_composite_curvature` call (dispatch returns the curvature for reuse) — fixed a `dense_jacobian` under-load timeout, byte-identical. Blast radius: 3 tests updated (`test_issue_267[exp(x-x*x)*y]` now sound via composite ownership; `test_flag_on_changes_nvs09` → graduation check; `test_convex_fast_path::…not_certified_optimal` — see below). **CI reconciliation (2026-07-12):** the graduated hull now relaxes `exp(-10000·(x-0.05)²)` (previously "cannot linearize" → feasibility fallback, no bound), so on `min -exp(-10000·(x-0.05)²)` the solve gains a valid dual bound (−1.0001549 ≤ true opt −1.0) and its local search reaches the global needle (obj −1.0) — while STILL not certifying (0.015 % gap open under 1e-6). The test's `objective > -0.1` assertion encoded the old *unrelaxable → weak incumbent* assumption; it was replaced with the true soundness invariant it guards (`status != "optimal"` AND `bound ≤ incumbent`), an improvement, not a weakening (maintainer-approved). The other CI failure that run — `test_m7_name_check_is_not_quadratic` — is a modeling-layer timing microbenchmark (variable-declaration cost, no relaxation code), a loaded-runner flake (passes locally), left untouched. **NEEDS LOCAL (CP-1):** full-corpus `incorrect_count=0` sweep + nvs09/global50 certification timing (the container is too slow to certify nvs09 in-budget). Defer-list *deletion* (replace the predicates' internals with canonical structure) is the next sub-step. |
