...
All anchors `python/discopt/_relax/milp_relaxation.py` (9129 lines) unless named.
...
- Extract `_relaxation_fingerprint` (test_lr2_offneutral_relaxation.py:44–78)
  into `python/discopt/_relax/claim_audit.py::relaxation_fingerprint(relax) ->
...
### R1.1 `python/discopt/_relax/canonical_expr.py` (library-only)
...
| EP2 OBBT single-build-per-box — premise FALSIFIED (already landed) | **DONE / NO-OP (2026-07-13)** | Entry measurement (per §0.3, run before implementing) **falsifies EP2's premise**: the per-node/root OBBT already builds the relaxation **once per (node box, round)** and shares it across all probes — the "each probe pays a full engine build" claim (EP0 profile §1) described the per-**round** `build_milp_relaxation`, not a per-probe rebuild. The reuse+warm-start EP2 specifies (`build once → swap objective per probe → warm-start from previous basis`) was **already landed by cert:T2.2 / PR #579** (`_PersistentProbeLP` in `obbt.py`, consumed by `run_obbt_on_relaxation`, reached from `obbt_tighten_root`), merged **2026-07-10 — three days BEFORE the EP0 profile (2026-07-13)** (git ancestry `git merge-base --is-ancestor 84abcb3 0a8a7885` = YES). **Measurement (instrumented `obbt_tighten_root`, hda 722 cols / 15 s budget):** `build_milp_relaxation` **calls = 1 per sweep** (builds/sweep = 1.00), that single build 6.83 s = **36.5 % of OBBT wall** (the rest = large-scale probe LP solves); **no per-probe build exists to remove.** The residual per-round engine build IS the cold rebuild **EP3's patch-table node path** targets, not EP2. **Outcome: no solver-math change** (byte-identity gate holds by construction — fingerprints on all 62, node counts + objectives unchanged); the one deliverable is a regression test `python/tests/test_obbt_ep2_node_reuse.py` (11 tests, 10 vendored instances) pinning the invariant at the *node* level — reuse+warm `obbt_tighten_root` boxes == forced-cold-seam boxes, Δ ≤ 1 ulp / ≤ 1e-9 — complementing T2.2's `run_obbt_on_relaxation`-level `test_obbt_warm_probes.py`. Inventing a rewrite of the working, byte-neutral-critical OBBT path to "land EP2" would be the speculative complexity §0.3 forbids; recorded and stopped. `ruff`/`format`/`mypy` clean; `pytest -m smoke` GREEN. |
| EP3 patch-table node path — fast path RESTORED for the engine; cheap-closed-form half FALSIFIED | **DONE in-container (2026-07-13)** | **Falsification (§0.3, measured before implementing):** the envisioned ~0.1 ms closed-form coefficient refresh is NOT byte-reproducible for the uniform engine — its box-dependent row coefficients + aux-column bounds are `evaluate_interval` results over the reconstructed DAG, and a closed-form recomputation differs from `evaluate_interval` bit-for-bit for any multi-term/folded/affine-arg atom (**measured:** fac2 3/3, alan 1/6 mismatches); the bare-variable closed-form `IncrementalMcCormickLP` validates on **0/62** engine-shaped instances. Byte-identity (EP3's whole soundness model) thus fails for the engine's dominant families, and single-atom replay cannot reproduce the interleaved aux-column layout ⇒ byte-identical replay == the full cold build. After EP1 the build is ~12 ms/node; the dominant per-node cost is per-node **separation** (EP4a), and the fast path's real lever is **skipping** it (inherited root pool) + **warm-starting** — the CC2 mitigation the #632 cutover disabled. **Delivered:** `UniformPatchTable` (`incremental_mccormick.py`) — `_patch` regenerates through the EP1-cached engine build (byte-identical by construction), engages only when the root objective bound is valid AND the lifted column layout is box-stable across reachable sub-boxes (`_validate`; else `ok=False` → cold path unchanged), and is wired ahead of the closed-form table in `mccormick_lp.py`. All existing guards preserved (T1.3 `out_cuts` skip, C-38 warm-basis re-solve, C-43 pool re-verify). **BOUND-NEUTRAL gate GREEN:** `relaxation_fingerprint` byte-identical on all 62 (no build path changed; `test_claim_baseline_neutral` green); nvs09 + ex1226 node counts + objectives **byte-identical fast-ON vs `DISCOPT_INCREMENTAL_MC=0` to optimality** (nvs09 19 nodes / −43.13433691803531; ex1226 5 nodes / −16.999999994); bchoco06 declines engagement (cold path unchanged). **Engagement 48/62 = 77%** (>50% target met). **Before→after (probe, fast engaged):** nvs09 **169→51 ms/node** (3.3×; the 51 ms — not the plan's ≤12 ms — is the direct consequence of the falsification: `_patch` still pays the ~12 ms build, so the win is separation-skip + warm-start); ex1226 3.6→2.4 ms/node. New `python/tests/test_uniform_patch_table.py` (engagement, byte-equal-to-cold at child boxes ×12 instances, unbounded-root declines, env toggle, sound-bound); `test_incremental_mccormick_node.py` closed-form-specific gates re-pointed at `IncrementalMcCormickLP` directly + 2 previously-deferred tests un-deferred (now engage). `ruff`/`format`/`mypy` clean. **Follow-up (out of EP3's soundly-deliverable scope):** a cheaper patch needs an engine that emits its coefficients in a byte-identical closed form, or a non-byte-identical differential-regime fast path; the 14 non-engaging instances are unbounded-root / box-unstable-layout. |
| EP4a multilinear facet cache — exact cache landed; measured wall win NEGLIGIBLE (§0.3 honest report) | **DONE in-container (2026-07-13)** | Exact memoization of `separate_multilinear_envelope` (`python/discopt/_relax/multilinear_separation.py`) in a capped module dict (`_FACET_CACHE`, 200k entries → clear-on-overflow), keyed by the **full** float64-bytes inputs `(lb, ub, x_star, w_star, tol, max_factors)` as a tuple of `bytes` slots (no delimiter collisions; `ascontiguousarray(...).tobytes()`, never rounded floats). **Plan-premise correction:** EP4a assumed the facet is box-only; measured, it is NOT — `_solve_envelope` returns the supporting facet *selected at* `x_star`, which varies with `x_star` on **10/16** nvs09 boxes queried at multiple points, so a box-only key would return a stale facet (a byte-neutrality bug). Keying on the full inputs is a pure-function memoization, byte-identical by construction (the plan's own fallback: "land only the exact cache"). **Closed-form arity≤3 NOT added** (correctly): no box-only closed form reproduces the `x_star`-selected, vertex-recomputed-intercept LP facet bit-for-bit, so the self-check would fail by definition. **BOUND-NEUTRAL gate GREEN:** node counts + certified objectives byte-identical before/after (nvs09 51 builds/19 nodes/−43.13433691803531; ex1226 24/5/−16.999999994161513; bchoco06 38/7/time_limit); `relaxation_fingerprint` byte-identical on all 62 (`test_claim_baseline_neutral` green, 113s); new `python/tests/test_ep4a_multilinear_facet_cache.py` (5 tests: cache-hit == fresh derivation byte-for-byte on 25 boxes/arity ×{2,3}×3 w_star; key-includes-x_star; no-resolve-on-hit; cap-clears). **Measured (nvs09):** cache deterministically removes **144/360 = 40% of facet-LP solves** (180 sep calls, 108 distinct inputs → 72 hits), but the recurring atoms are the *cheap* LPs — `_solve_envelope` cumulative time 15.1→14.6 s (~3%), within noise at the 45 s solve; EP0 ms/node unchanged (53.0→52.7, distinct cold boxes have no recurrence). **Win negligible on the certifying instances** (the multilinear hull LPs are already cheap post-EP1/EP3 where they recur; the heavy per-node separation cost is `_separate_convex`'s JAX-grad Kelley loop, EP4b/EP5's target) — landed anyway as the guaranteed-neutral, harmless, bounded removal of redundant work; no manufactured win. `ruff`/`format`/`mypy` clean. |
| EP5 lazy + shared JAX compiles — jit falsified non-neutral; byte-neutral `eval_jaxpr` landed | **DONE in-container (2026-07-13)** | The 2026-07-13 profile put the single largest avoidable cost in the BARE `jax.grad` `grad_fn` of `mccormick_lp._separate_convex`: it RE-LINEARIZES (re-traces autodiff) on every call, up to 8 rounds/spec/node, in interpreted JAX. **Entry experiment (§0.3, before landing):** bit-for-bit test of two fixes vs eager on the ACTUAL lift points (nvs09/tspn05 full solves, 311 value+grad comparisons). **`jax.jit` is NOT bit-identical** — XLA fusion reorders float ops: nvs09 5/286 value bits Δ≤7.1e-15, tspn05 5/25 value Δ≤1.1e-12 **and 6/25 grad** Δ≤2.4e-15 → jitting is **bound-CHANGING** (differential regime EP4b/EP6), NOT landed. **Pre-trace-once → `jax.core.eval_jaxpr` IS bit-identical** (0/311 value+grad) and **5.7×** on the grad path (same op-by-op primitive dispatch as eager, no fusion) → landed. **What landed (`uniform_relax.py` only):** `_compiled(node)` wraps value/grad in a lazy `_TracedEvalFn` — `compile_expression` (cheap closure, no trace) stays eager so the **lift/no-lift decision is byte-identical**, but the autodiff trace is deferred to the first `_separate_convex` use and reused (`eval_jaxpr`) for the model's life instead of re-tracing every round. **Lazy**: a lifted spec that never separates never traces (pays nothing). **Exception-transparent**: a first-use trace failure raises where the eager call did → `_separate_convex` skips that spec (same sound no-op). **Hash-consing (verified):** `CanonicalDAG._mk` interns structurally identical CNodes to one object and `_compiled` keys on `id(cnode)`, so identical occurrences already share one wrapper. **Ctor double-build NOT touched** — `UniformPatchTable._validate`'s second `_full_build` IS EP3's byte-identity guard, not free waste; left to EP3's path per the "only if trivial" directive. **BOUND-NEUTRAL gate GREEN:** `relaxation_fingerprint` byte-identical on all 62 (`test_claim_baseline_neutral`, 71 s); node/obj byte-identical before/after AND fast-ON vs `DISCOPT_INCREMENTAL_MC=0` (nvs09 19/−43.13433691803531, ex1226 5/−16.999999994161513, bchoco06 7/time_limit); 3 new `test_uniform_relax.py::test_ep5_*` (byte-identity+laziness+hash-consing); `pytest -m smoke` 620 passed; ruff/format/mypy clean. **Measured (nvs09 `m.solve()`):** wall **44.9→32.8 s** (−12 s), AD cProfile tottime (`ad.py`+`linearize`) **3.23→0.085 s** (one-time `make_jaxpr`+`eval_jaxpr`≈0.73 s reused), 19 nodes / obj −43.13433691803531 unchanged; EP0 ms/node unchanged (53.1→52.0, child boxes don't run the grad loop — win is in full-solve wall). Commit `perf(ep5)` on `claude/issue-632-opus-plan-ffxld4`. |
| EP4b separation warm-start + OA pool — all three sub-changes FALSIFIED by the entry experiment; NO-OP | **DONE / NO-OP in-container (2026-07-13)** | Entry experiment (§0.3, before any code): one nvs09 `m.solve()` (32–37 s, obj −43.13433691803531, 19 nodes), cProfile'd, with the `_separate_convex` Kelley loop instrumented for per-round vstack vs LP-solve time and warm-basis state. All three EP4b sub-changes have **no available win**: **(A) incremental append vs full `vstack`** — the entire scipy-sparse construction stack costs **0.05 s of 37 s** (`_compressed_sparse_stack` 438 calls / 0.052 s; `_separate_convex`'s own `_append` vstack 0.035 s across 96 rounds) → < 0.2 %, negligible, dropped per §0.3 (no speculative complexity for a 0.05 s "win"). **(B) warm-start the round-to-round re-solve** — ALREADY implemented: relaxer default `backend="simplex"` routes every Kelley re-solve through `MilpRelaxationModel._solve_lp_warm` (reuses the prior optimal basis when rows only grow); instrumented **warm_hits = 96/96 (100 %)** on nvs09 → nothing to add, and it is byte-neutral (warm dual simplex returns the same LP optimum). **(C) OA cut pooling for child inheritance** — the premise "engine OA cuts are not pooled the way RLT/PSD are" is FALSE: the general root cut pool (`solver.py:5319`, gated on `_mc_lp_relaxer._inc is not None`; nvs09 engages the incremental engine per EP3) captures the FULL separation chain including convex tangents (`solver.py:5326` names "convex"; `mccormick_lp.py:1417` captures every appended row) and inherits it at every fast-path node — confirmed at runtime: *"Root cut pool (general spatial): 175 cuts (of 175 separated), inherited at every fast-path node."* The only unbuilt variant — per-node **parent→child** tangent pooling — is bound-CHANGING, needs an invasive B&B-tree rewrite (driver threads one global `_root_cut_pool`, not per-parent pools), targets the non-dominant cost (`_separate_convex` 8.4 s is ~90 % JAX grad eval = EP5 territory, not LP 0.9 s / vstack 0.035 s), and has NO measured evidence of a net win → deferred (not implemented speculatively; graduates on the local host if a global50 profile shows a class where inherited parent tangents cut node counts, then behind a default-off flag with the differential regime, like EP6). **Outcome: no library change** ⇒ byte-identity on all 62 + node/objective equality on the certifying set hold trivially (nvs09 obj −43.13433691803531 / optimal, unchanged from EP5-HEAD); differential regime moot (nothing bound-changing to gate). Dominant nvs09 costs (multilinear separation 14.5 s = EP4a-negligible; Rust LP 14.8 s = genuine simplex work on distinct cold boxes) are outside EP4b scope and are not redundant computation EP4b could remove. Docs-only (this row + plan §3/EP4b section); `pytest -m smoke` + ruff/format/mypy re-confirmed green. |
...
| PF0 panel outcome + differential harness (#632) | **DONE in-container (2026-07-14)** | `discopt_benchmarks/scripts/pf_panel.py` — measurement-only, no library change, no new flag. Three modes: (1) **panel** — 62 vendored `.nl` end-to-end, subprocess-isolated (`--jobs N`; one hang killed at `budget+grace`, env flags read fresh per worker), emits per-instance `{status,objective,bound,nodes,wall,sense}` + summary `{proved,feasible,timeout,total_wall}`; (2) **diff** (`--vs REF`) — sense-aware proved-Δ/node-ratio/bound-direction; exits non-zero on any LOOSER dual bound or a bound that CROSSED the reference objective (EP3-catcher, verified firing on synthetic looser/crossed rows); (3) **differential** (`--env-a/--env-b`) — root+child-box bound comparison (reuses `engine_perf_probe._child_boxes`; asserts env-b at-least-as-tight per box, sense-aware) + feasible-point soundness sampler (reuses `uniform_engine_validation._sample_soundness`, 0 cuts required). Baseline committed at `docs/dev/data/pf-baseline.json` (HEAD, 30 s/instance) as the standing PF-series reference; the fast in-container stand-in for global50. ruff/format clean; 5-instance smoke green. |
| PF1 branch-and-reduce ON — global-loop wiring LANDED (#632) | **DONE in-container (2026-07-14)** | The spike (`docs/dev/pf1-branch-and-reduce-spike-2026-07-14.md`) found the in-tree FBBT/reduce kernel wired ONLY into `_solve_nlp_bb` (`node_depth` hardcoded 0), so the GLOBAL spatial B&B path — every unproved instance (tspn*/bchoco*/heatexch*/nvs05) — never invoked it (call-count 0). **Landed:** (1) wire the persistent Rust `run_in_tree_presolve` kernel into the global spatial node loop (`solver.py:~6134`), passing the real node box + incumbent-as-cutoff + real per-node depth; (2) new read-only `TreeManager::node_depth` (`tree_manager.rs`) + `PyTreeManager.node_depths` batch binding (`bnb_bindings.rs`) so the depth-stride gate is honest on both paths (replaces `node_depth=0`); (3) flip default `in_tree_presolve_stride` 0→1 on BOTH `solve_model` and `_solve_nlp_bb` (FBBT-only; `DISCOPT_NODE_PROBING` stays default-OFF per the spike's net-negative probing evidence); (4) fix the `_note_ignored_mip_nlp` default-comparison at `solver.py:3396` (`!= 0`→`!= 1`) so a default mip-nlp solve no longer spuriously warns. **Soundness (absolute):** every contraction is an outward-rounded interval INTERSECTION with the node box (the kernel never loosens); a proven-empty box fathoms the node; a reduced box always still contains the entire feasible region — no feasible point, never the optimum, is excluded. Block-count guard skips the reduce when a reformulated repr's var count ≠ node-box length (intersect-only floor keeps it sound even then). **Telemetry + pin:** module counter `_in_tree_presolve_global_calls()` (reset per `solve_model`), regression test `python/tests/test_pf1_global_in_tree_presolve.py` asserts the global loop fires >0 at stride≥1 and 0 at stride=0 on an inline nonconvex bilinear MINLP (class, not a named instance). Confirmed firing: nvs05 150 / tspn05 29 / bchoco06 1 / st_e38 3. **Measured (controlled OFF-vs-ON, identical binary, 11-inst spike set, 40 s, jobs 4):** m3 **feasible→OPTIMAL** (proof, 61→47 n), fac2 **−43 %** (69→39 n), clay0303hfsg **−28 %** (223→159 n) — NLP-BB stride flip; GLOBAL path: **nvs05 dual bound 1.3521→4.3023 (3.2× tighter, sound bound ≤ obj 5.47)**, tspn05 bound flat (39→37 n), bchoco06/07+heatexch_gen1/2 unchanged (stuck ≤3 nodes at TL — pre-reduction bottleneck). **Gates GREEN:** `--vs pf-baseline.json` (m3 proof gained, 0 lost, no LOOSER/CROSSED); differential (7 inst) env-b at-least-as-tight per box + feasible-point worst 1.8e-11, **0 cuts**; `bound ≤ objective` 0 violations on all rows. `pf-baseline.json` regenerated at ON default (**full 62-inst corpus 30 s: proved 36→41, +5 [cvxnonsep_nsig30, m3, nvs02, nvs11, st_e36], 0 lost, no LOOSER/CROSSED, hangs 5→1, wall 1067→963 s**). cargo test `discopt-core` 445+4+1; smoke 634; fast selection 5271 passed/0 fail; claim_boundary 360; bilevel+GDP soundness 17 (rebased #638 holds); ruff/format/mypy clean; `maturin develop` rebuilt. **Verdict:** sound, no-regression positive (nvs05 3× tighter bound) but NOT the sole BARON lever on the hardest stuck spatial instances — the biggest wins are the NLP-BB stride flip; range reduction alone doesn't move bchoco/heatexch, whose bottleneck is upstream (relaxation/branching/first-incumbent). Commit `perf(pf1)` on `claude/issue-632-opus-plan-ffxld4`. |
| PF4 LMTD / log-mean envelope (heatexch class) | **FULL ITEM → KILL on SOUNDNESS (2026-07-14)** | The PF4 spike proposed a `GM ≤ LMTD ≤ AM` envelope for heatexch's Log-Mean Temperature Difference atoms. **Falsified on the hard soundness gate:** the spike sampled the ε-FREE mean `(a−b)/log(a/b)` (which obeys `GM≤·≤AM`); the model term is `(a−b)/log(a/(ε+b))` with ε=1e-6, a,b∈[10,∞). The `+ε` does not remove the LMTD pole — it MOVES it to the line `a=ε+b`, strictly INSIDE the box (e.g. a=10.000001, b=10), where the denominator crosses 0 and the exact aux value `w` is genuinely unbounded (±∞). Sampling the EXACT term over [10,650]² (2M pts): AM `w≤(a+b)/2` **cut 3211 feasible points** (worst +5.10), GM **cut 3971** (worst +3.78) — feasible-point cuts, the worst class of bug. A "sign-definite denominator" gate is INSUFFICIENT (AM still violated 417 707/3 000 000 on a denom>0 pole-hugging box; the validity margin is quantitative + box-dependent and fails at the pinch region where good HEN solutions live). Because `w` is truly unbounded on any pole-straddling box, the uniform engine's current "ratio → unbounded interval floor" is the SOUND answer, not a fixable hole; the AM over-estimator (the only one that could tighten heatexch's dual, area-cost min drives `w` large) is precisely the unsound one, and the sound GM under-estimator gives 0 root improvement in the binding direction. **Landed:** nothing in the relaxation; a guard test `python/tests/test_pf4_lmtd_epsilon_pole.py` pins the falsification (in-box pole; AM/GM unsound; sign-gate insufficient; current uniform relaxation cuts no near-pole feasible point). Entry-experiment roots unchanged (gen1 38 184/75.3%, gen2 543 496/14.5%, gen3 43 888/32.3%). Architecture note: the default relaxation is the uniform factorable engine (`uniform_relax.py`), NOT the federated `build_milp_relaxation`/`terms.general_nl` path the spike named — the ratio atom is already relaxed there (to the correct unbounded floor). Commit `feat(pf4)` on `claude/issue-632-opus-plan-ffxld4`. |
