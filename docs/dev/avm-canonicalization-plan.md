...
| P1.1 atomize composite univariate `f(x)^p` (the AVM square atom) | **DONE in-container (2026-07-13)** | **Root cause (file:line):** the composite `(ln(x-2))**2` **is** atomized on the default relaxation path — `build_milp_relaxation` allocates the inner aux `w = ln(x-2)` (exact concave log envelope, `_collect_univariate_relaxations`) **and** the outer square `s = w**2` (`_collect_univariate_square_relaxations`, `milp_relaxation.py:4837`, issue #369; static convex envelope emitted at `:8333`). The bound was still 0 because the LP-point tangent separator `MccormickLPRelaxer._separate_univariate_square` (`mccormick_lp.py:1716`) built its `(base,aux)` spec list **only from `varmap["monomial"]`** (`:1736–1742`) — it skipped the `univariate_square` column family. So the outer square kept its loose static envelope (tangents only at `w`'s endpoints+0), which under-cuts `s=w**2` to 0 in `w`'s interior. `factorable_reform` (`_prelift_call_powers`) masked this only by rewriting `g(x)**2` into a bare-variable **monomial** `t**2`, which the separator already covered — hence the reform path already reaches 1.89 while the un-reformed relaxer stays 0. **Fix:** extend `_separate_univariate_square`'s specs to also iterate `varmap["univariate_square"]`, adding the identical exact convex tangent `s ≥ 2·x0·w − x0²` (a global underestimator of `s=w²` for any base column ⇒ `(w−x0)²≥0`, cuts no feasible point) to the composite atom columns. No new envelope math; structure-keyed (the univariate_square map), no instance names. **Measured recovery:** nvs09 per-var composite `(ln(x−2))²+(ln(10−x))²` on [3,9] **0.0 → 1.89328** (= the AVM tier, 51.6%/var; sound ≤ true 3.66667); nvs09 squares-only (10 vars) **0.0 → 18.93283** (≈10×, ≤ true 36.66673). **Differential over 62 vendored `.nl` (root LP via in-house simplex, new vs stashed-old):** **0 dropped (no regression), 0 net corpus tightening, 46 unchanged, 16 both-None** — the *only* corpus instance producing `univariate_square` columns is nvs09 (20 cols), whose objective is masked (`objective_bound_valid=False`) by the un-atomizable `(∏xᵢ)^0.2` product term, so its net root bound stays None until Target 3 (log-space) lands. **Feasible-point sampling:** 2000 samples each on the probe (38 rows) and the 10-var squares model (380 rows) against the **separated** relaxation → **0 violations, max row violation 0.0**. **Regression test:** `test_univariate_square_separation.py::test_composite_univariate_square_separation_recovers_avm_bound` (fails pre-fix at the ≥1.89 assertion, passes post-fix; soundness ≤3.66667 asserted). smoke 645p/12s, adversarial 10p, `_jax` ruff/format/mypy clean. **Local-host handoff:** end-to-end `incorrect_count=0` full-corpus certification and the end-to-end nvs09 certified-root timing need BARON/`minlplib.solu` (absent in-container); and the corpus-visible payoff of this atom is gated on Target 3 unblocking nvs09's finite objective bound. |
