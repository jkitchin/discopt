...
| **3 ✅ DONE** | `saa.py` (Mak–Morton–Wood SAA lower/upper statistical bounds + optimality-gap estimate), `dro.py` (discrete DRO worst-case distribution for a total-variation ambiguity ball + the `ro/` bridge), `multistage.py` (nested-Benders + integer-L-shaped **stubs that refuse loudly**) | SAA one-sided CIs are **coverage-calibrated** over 3000 synthetic trials (P(bound ≤ μ) ≈ conf); the DRO worst-case cost **matches a scipy LP oracle** across 25 random instances (budget-0 = nominal, full-budget = all mass on max-cost); reserved methods raise — **8 tests pass, ruff clean**. Multistage/integer-recourse certified solves are deferred (stubs point at `build_extensive_form` / the `NESTED_BENDERS` slot). | notebook (docs iteration); `references.bib` entries |
