...
| **FR-1 (C-34)** | **P0 (default path)** | `gdp_reformulate.py:493-506` (`**` arm) | **Even-power bound over a zero-straddling base uses endpoint-only bounds.** Only `p==2` gets the straddle-aware `[0, max(lb²,ub²)]`; `p≥4` over a base whose box straddles 0 gets `[min(endpoints), max(endpoints)]`, omitting the interior min at 0. `_bound_expression(x**4)` on `[−2,2]` → `[16,16]` (true `[0,16]`). Feeds the aux box (`factorable_reform.py:254`) and denominator clearing (`:707`). | VERIFIED e2e — `min (x−0.5)²+(y−1)² s.t. x**4·y ≤ 1` returns obj 0.25 at x=1; true 0 at x=0.5 |
