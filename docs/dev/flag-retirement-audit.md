...
| `DISCOPT_TRIVIAL_PRIMAL` | #827's trivial-point primal seed (origin / box-center / bound corners, verified against the true constraints and re-verified downstream by ``ws_int_feas`` before injection). Its gate docstring promised a §5 panel that had never been run, and the flag had no row here at all -- it is read with a `""` default, so the scan behind the marker above cannot see it (that blind spot spans three read forms and is filed separately) | **panel RUN 2026-09-22** (#1422), `discopt_benchmarks/scripts/issue1422_trivial_primal_graduation_panel.py`, 66 vendored instances x 2 arms at 8 s, interleaved within each instance with the arm order alternated by index, `deterministic=True`: **426 executed checks, 0 violations** -- no bound above its incumbent or above `cert-optima.json`, no certification lost, no objective drift, every incumbent through the shared false-primal screen. Net-positive: **0 incumbents gained, 0 lost** | **RESOLVED -- keep as a documented opt-out.** BAR 1 (cert-clean) passes; BAR 2 cannot be scored on this corpus, and that is *measured*, not assumed. A structural probe replaying the seed's own candidate ladder over all 66 instances (264 executed candidate checks, no solve) finds a trivial feasible point on **25 of 66** -- and on **0 of the 14** instances whose default-path solve returns no incumbent. The two sets are **disjoint here**, so every instance the seed can fire on is one that already solves, where it is a no-op by construction; the panel measuring nothing is the corpus's property, not the flag's. This is the `DISCOPT_BLOCK_VECTOR_EVAL` shape. It is not a retirement candidate: on the class it was built for the gain is direct and reproducible -- the `ball_mk2_30` shape (30 integers in [-1,1], linear objective, one convex ball row) at 8 s goes `objective=None` OFF to **`0.0` ON, the reference optimum**. **What would change it:** a panel over the MINLPLib snapshot restricted to the intersection that is empty in-repo -- instances the default path leaves with no incumbent AND on which a trivial candidate is feasible. Note that `ball_mk2_30` itself cannot be run at the DEFAULT configuration: it is convex-kernel eligible, the attempt takes `min(time_limit, DISCOPT_CONVEX_KERNEL_BUDGET)` (the whole budget for any `time_limit <= 120`), declines, and leaves `solve_model` ~0 s, so such a panel must pin `DISCOPT_CONVEX_KERNEL=0` or use a budget above 120 s (#1422) |
| `DISCOPT_CONVEX_KERNEL_KEEP_BOUND` | #1422: `Model.solve` adopts the rigorous dual bound a **declined** convex-kernel attempt already proved. The attempt takes `min(time_limit, DISCOPT_CONVEX_KERNEL_BUDGET)` — the whole budget for any `time_limit <= 120` — so on a model it declines the default path gets ~0 s and reports `bound=None`, while the kernel had proved one and discarded it. Entry measurement over the 39 kernel-eligible MINLPLib-snapshot instances at 8 s: **82% of all kernel wall (152.0 s of 185.4 s) goes to models it declines**, 15 of those 20 carrying a finite discarded bound | **panel RUN 2026-09-22** (#1422), `scratchpad/ck_panel1422.py`, 39 instances x 2 arms at 8 s, interleaved within each instance: **73 executed comparisons, 0 violations** — no bound above its reference optimum (`minlplib.solu`), no certification regression, no status change, no objective drift, no bound made looser. Net-positive: **16 of 39 gain a dual bound where the OFF arm has none**, 0 lost, 0 loosened; total wall 190.7 s both arms (**-0.0%**) | **GRADUATED ON INTRODUCTION** — default ON, `=0` opt-out and the legacy path kept per §5. Same shape as `DISCOPT_FARKAS_RAY_CLEANUP` and `DISCOPT_TREE_SENTINEL_PRUNE_GUARD`: the panel is in the PR that introduces the flag, so the gate is never in the stalled state §5's clause names. The wall column is the reason the net-positive bar is so clean — the mechanism reads a number the tree already computed and takes no time from the attempt, which is exactly why it is viable where the two *allocation* fixes are not (the fractional cap was rejected under #911; abandon-on-no-incumbent was falsified for #1422 — the kernel finds its first incumbent essentially at convergence). Soundness rests on evidence rather than #779's cross-check, which a declined attempt cannot supply: only `time_limit`/`node_limit` outcomes publish at all, the `BOUND_INF` sentinel is screened, the bound is never used to prune, and it is refused loudly if it crosses an incumbent the solve found. **#1422 stays open**: item 4, the allocation itself, is untouched by this flag |
| `DISCOPT_CONVEX_KERNEL_KEEP_INCUMBENT` | #1440: `Model.solve` adopts the **feasible point** a **declined** convex-kernel attempt already found — the incumbent counterpart of `DISCOPT_CONVEX_KERNEL_KEEP_BOUND` above, and the other half of the same waste. The attempt takes `min(time_limit, DISCOPT_CONVEX_KERNEL_BUDGET)` (the whole budget for any `time_limit <= 120`), so on a model it declines the default path gets ~0 s and reports `objective=None` while the kernel was holding a verified point. Entry measurement on `clay0303hfsg`, the one in-repo member of the class, 5 replicates per budget: the attempt holds a point **0/5 at 10 s, 4/5 at 12 s, 5/5 at 14 s** — always 47287.5613, verified feasible against the pristine model, above the reference optimum 26669.11 (valid, not a false primal), while the same attempt proves 23239.6–25496.4 | **panel RUN 2026-09-23** (#1440), `scratchpad/k1440/panel1440.py`, in TWO parts because the standard 8 s panel budget is *below* the window the entry table measures and would otherwise score a mechanism that never fires. *Exposure*, 66 vendored instances x 2 arms at 8 s, interleaved within each instance: **302 executed comparisons, 0 violations** — no bound above its reference optimum (`known_optima.toml`), no incumbent below one, every reported incumbent independently re-verified against a fresh parse, no certification regression; **0 gains, 0 losses**; total wall 246.0 s ON vs 245.2 s OFF (**+0.3%**). *Benefit*, the 3 kernel-eligible instances x 2 arms at 14 s: **14 executed comparisons, 0 violations, 1 gain, 0 losses** (`clay0303hfsg` 47287.5613 where OFF reports `objective=None`), wall +0.2%. The one differing row in the 8 s panel (`tanksize`, `bound=None` ON vs 0.887 OFF) is budget-boundary variance on a model `build_convex_spec` refuses, where this flag cannot fire: a 3x2 control reproduces the swing in **both** arms (ON 3/3 bounded, OFF 1/3; 0–36 nodes within one arm) | **GRADUATED ON INTRODUCTION** — default ON, `=0` opt-out and the legacy path kept per §5. Same shape as `DISCOPT_CONVEX_KERNEL_KEEP_BOUND`: the panel is in the PR that introduces the flag, so the gate is never in the stalled state §5's clause names. **Soundness is stronger here than for the bound, not weaker.** A dual bound can only be *inferred* — `KEEP_BOUND`'s row rests on an oracle panel because a declined attempt cannot supply #779's cross-check. A point is *checkable*, and this one is checked twice against the PRISTINE model before it can be reported: by `_incumbent_is_feasible` (the full row-map verifier, #779) at the publication site, and again by the #772 false-primal screen, which covers the adopted point with no change of its own. Verification failure publishes nothing (fail closed), `gap_certified` is never set, the status is never upgraded, and three contradictions are refused *loudly* rather than resolved silently (§3): a point against a `status="infeasible"` proof, against a certified optimum, and against a valid dual bound it would cross. Like `KEEP_BOUND` it takes no time from the attempt, so it cannot cost a certification the way #911's fractional cap and #1422's abandon-on-no-incumbent both do. **#1440 stays open on the allocation itself**, which neither recovery touches |
...
| `DISCOPT_LP_KAPPA_RECOVERY` (Rust, `lp/simplex/dual.rs`) | The dual optimality gate decided whether to re-solve `x_B` before **certifying** it from `growth()` alone — `‖U‖∞/‖U₀‖∞`, a measure of what *elimination* did, which is exactly 1.0 on `diag(1, 1e-10)` however singular the basis is. κ₁ (Hager–Higham) was already wrapped as `FeralLU::condition_estimate()`, whose docstring claimed it was "the signal that drives in-engine recovery (refine / perturb / branch)"; it had **zero production callers** for its whole life, because the wrapper demanded `build_basis_matrix()` → the numeric-focus `O(m²)` retained copy that `node_feral_lu()` never builds, so it returned `None` at every site that wanted it. feral's own estimator takes the basis **as an argument** — one `O(nnz)` pass plus `≤ 2·5+1` ftran/btran against the factor already in hand — so "dead code with a false docstring" and "unaffordable signal" were the *same* defect. The false claim is retracted in place per §11. Entry experiment over the 66-instance `minlplib_nl` corpus at 20 s, **335,033 confirmation points**: 677 (0.20%) trip growth today, while **13,749 (4.10%)** sit at growth ≤ 1e4 with κ₁ > 1e10 — worst at growth **exactly 1.000** with κ₁ ≈ 7.1e18 (`κ·u ≈ 1.6e3`, not one correct digit). The same data killed the textbook line: `κ·u = tol` gives `κ ≈ 4.5e6`, but the corpus **median** κ₁ is 1.39e6 and 52.0% exceed 1e6, so that trigger would run recovery on half of every solve; `KAPPA_REFINE_TRIGGER = 1e12` is read off the measured exceedance curve (1e8 → 19.1%, 1e10 → 4.10%, **1e12 → 2.66%**, 1e14 → 0.92%; ≈ p97.3) | **panel RUN 2026-09-26** (this PR), `discopt_benchmarks/scripts/kappa_recovery_graduation_panel.py`, 66 vendored instances × 2 arms at 20 s, interleaved with the arm order alternating, **each arm in its own subprocess** (see the retraction below). BAR 1 (*cert-clean*) **passes**: 543 executed checks, 0 violations, certified **48/48 in both arms**, 0 status or certification disagreements, every incumbent through the shared false-primal screen. Fired provably (§6): **10,818 κ₁-caused refined recomputes over 12 instances, 5 of them rescues** — the refined `x_B` primal-infeasible where the working `x_B` looked feasible, i.e. 5 averted optimality confirmations from a basis with under four correct digits; `nvs05` ON reaches the true global optimum (5.470934111 vs. reference 5.470934108) where OFF finishes **7.6% above it**. BAR 2 (*net-positive*) **not met**: nodes +0.72% (5827 → 5869), wall +2.80% (428.5 s → 440.6 s, one replicate per instance at load average 4–10, so indicative not precise, §9). The one loosened dual bound (`heatexch_gen2`, 566237.076 → 558540.173) is **not** the flag: both arms show `att=0`, same objective, same 63 nodes, and a 6-replicate interleaved control finds it bimodal **within** the OFF arm with the within-OFF spread **exactly** the OFF↔ON gap (7696.9029) — the `tanksize` shape from the #1440 row above. Cost outlier to know: `clay0303hfsg`, +10,194 attempts, ≈54 refined recomputes per node on a 189-node solve | **Keep as a documented opt-in.** BAR 1 passes and the mechanism demonstrably fires and rescues, but the OFF arm produced **no wrong answer on this corpus**, so ON buys removal of a *latent* risk at a measured cost, which is negative against §5's net-positive bar as written. The `DISCOPT_TREE_SENTINEL_PRUNE_GUARD` row above graduated a soundness guard over exactly this objection — §1 governs, not §5 — but it had a **demonstrated false certificate** behind it (`gams01`-style) and this does not; graduating on a latent risk alone would make "soundness guard" a phrase that exempts any flag from measurement. This is explicitly **not** a stalled graduation: the panel is in the PR that introduces the flag, and the gate's docstring carries the result rather than a promise. **Two findings flip it, and both are cheap to look for:** (1) a corpus instance where the OFF arm certifies a bound the oracle refutes — then it graduates as a soundness guard and the wall cost stops mattering; (2) an estimate cadence that erases the +2.8% — re-estimate only after a refactorization or past an eta-count threshold instead of at every confirmation point, which is precisely what `clay0303hfsg`'s 54-per-node is paying for. Until then the κ₁ path is reachable, tested, and one env var from being measured again by anyone. **Retraction (§11, §6), recorded because the instrument was the thing that failed:** the first draft of this panel ran both arms **in one interpreter** and reported **0 κ₁-caused recoveries at threshold 1.001** — where `κ₁ > 1.001` holds for *every* basis. `kappa_refine_trigger()` caches in a Rust `OnceLock`, so the first arm pinned the value and **both arms were the OFF arm**. Believed, that run would have reported a clean "no effect" for a mechanism it never enabled — the `parse_stall_patience` lesson reached from the other side. The panel now forks per arm and the fix is proven, not asserted: at threshold 1.001 on `gkocis,m3` the ON arm shows **156** extra `RefinedRecoveryAttemptsDual` against **0** OFF, and `_solve_once`'s docstring records why the subprocess is mandatory |
...
| flag | what its own text records | state | verdict |
|---|---|---|---|
| `DISCOPT_PUMP_GATE_ALIGN` *(retired 2026-09-23)* | #1449. The feasibility pump's accept gate is a **pre-filter**, not a guard: every candidate it passes is re-verified by `solver._native_kernel_verify_point` -> `validation.feasibility.verify_point` before it may seed a cutoff or become an incumbent. But it holds candidates to `tol + rtol*scale` (rtol=1e-9) where that judge allows `abs_tol*max(anchor, scale)` — on a row whose terms are of magnitude 3.2e4, 3.26e-5 against 3.16e-2, a **~970x gap**. ON, the loosening half became `max(legacy, abs_tol*max(1, scale))` | **entry experiment on the real class**: `verify_point` re-run on every rejection the gate made during a real solve — **8 of 57 compared rejections (14.0 %)** on a 119-instance MINLPLib sample were points `verify_point` accepts outright, 7x the issue's 2 % kill criterion (1 of 62, 1.6 %, on the vendored corpus). **Graduation panel, 117 instances at 20 s, arms interleaved:** gate 1 PASS — 0 bounds above a reference optimum, 0 certification regressions, 0 unverified incumbents in either arm. Gate 2 **FAIL** — 30/30 certified, 61/61 with an incumbent, total wall +0.15 %, dual bound 6 tighter / 8 looser, arms differing on 19/117. Its single objective mover (`autocorr_bern40-20`) did not reproduce: 8 interleaved repeats per arm gave OFF −50223.5 ± 158.0 and ON −50236.0 ± 117.8, the best of all 16 runs in the **OFF** arm | **RETIRED.** Flag and implementation deleted in one PR (#1449). Cert-clean but not net-positive — the `DISCOPT_CUT_INHERIT` outcome, taken to its exit rather than left as a stalled default-OFF gate. The measurement that killed it is preserved in `combined_tolerance`'s docstring and in `test_1449_pump_gate_alignment.py`, which now pins the divergence as deliberate. Reopen only on evidence of a primal **loss** attributable to the threshold, not of a rejection count. |
