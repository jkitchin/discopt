...
The five that remain: `COEF_TIGHTEN` (graduation candidate, benefit measured),
...
| `DISCOPT_COEF_TIGHTEN` | `docs/dev/issue-282-stage2-verdict.md`: *"Coefficient tightening is the load-bearing lever exactly as Stage 0 predicted"* — `syn40m` root gap **+2608 % → +1145 %**, 52-62 rows tightened on `rsyn*` | **benefit measured**, panel not run | **RESOLVED — graduation candidate, NOT a retirement one.** Corrected 2026-09-20. The 2026-09-19 pass read only "until a panel graduates it" and filed it as *panel owed*; its own verdict doc records a large measured gain. What that doc FALSIFIES is **Stage 2** (VUB + sustained aggregation), not Stage 1, which is this flag — do not read its banner as a verdict here. Needs an `ARMS` entry, then `graduation_gate.py`. |
