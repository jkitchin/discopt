...
| flag | what its own text records | state | verdict |
|---|---|---|---|
| `DISCOPT_PUMP_GATE_ALIGN` | #1449. The feasibility pump's accept gate is a **pre-filter**, not a guard: every candidate it passes is re-verified by `solver._native_kernel_verify_point` -> `validation.feasibility.verify_point` before it may seed a cutoff or become an incumbent ("a heuristic's own reported objective is never trusted directly"). But it holds candidates to `tol + rtol*scale` (rtol=1e-9) where that judge allows `abs_tol*max(anchor, scale)` — on a row whose terms are of magnitude 3.2e4, 3.26e-5 against 3.16e-2, a **~970x gap**. ON, the loosening half becomes `max(legacy, abs_tol*max(1, scale))`, so it never tightens and never materially outruns the judge; the #1254 distance cap is shared by both arms and untouched | **entry experiment on the real class**: `verify_point` re-run on every rejection the gate made during a real solve — **8 of 57 compared rejections (14.0 %)** on a 119-instance MINLPLib sample were points `verify_point` accepts outright, **7x the issue's own 2 % kill criterion**; 1 of 62 (1.6 %) on the 66-instance vendored corpus | **Panel owed — running.** Soundness rests entirely on the downstream verifier, which this does not touch, so §5 gate 1 is nearly free and the open question is gate 2. Note the ±1-instance noise floor recorded in `budget-monotonicity-1153.md` §6.8c: 73 % of that corpus runs to the wall, so any mover must be repeated in both arms before it is believed. |
