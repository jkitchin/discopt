...
## §7. Consolidation / stopping point (2026-07-14)

The TX series is paused at a clean stopping point. Honest scorecard: the
codified metric (easy-class geomean vs BARON) is **unmoved** — it is
floor-blocked (TX3), and every other reachable lever proved engine-scope
(native deadline, dual evaluator, OBBT filtering), not a Python item. What the
series produced is durable *findings* (this doc, §2 + the TX rows) and one
gated win, not a 10×.

**Kept (default-ON, the PR's actual value):** the federation cutover (uniform
engine), PF1 (FBBT range-reduction, panel 36→41), F1 (pinned-dim dedup, nvs09
64→28 s). These are real and stand on their own.

**Kept (gated default-OFF, sound + tested, await nightly graduation):** F2′
(`DISCOPT_ANALYTIC_SEPGRAD`, deterministic sep-grad, +1 proof), TX1
(`DISCOPT_ADAPTIVE_NLP`, +2 proofs / −45 s). Off by default → cannot regress
anything; graduate via `flag-graduation-protocol.md` when validated.

**Cut:** TX2b native-deadline mechanism (inert — no byte-neutral in-repo
caller; reverted, in git history at `b8c591f`).

**Deferred findings (not this PR):** the easy-class floor needs a dual
numpy/JAX evaluator (TX3); casctanks/bchoco08 need OBBT-probe sub-budgeting or
a dual-evaluator; heatexch_gen3 needs a native POUNCE deadline; hda's HEAD hang
is a relaxation-build (`_try_convex_lift`) cost, not a deadline item; the
AMP dense-partition guard (`_MAX_RELAXATION_PARTITION_INTERVALS`, now dead
code) needs verify-or-restore (tracked). The integer-product family (TX4/TX5)
is real SCIP-gap work but barely observable in-container — its win is on the
out-of-panel nvs17/19/24 + graphpart.

**The honest conclusion:** SOTA-fast is a separate, scoped engine effort — a
dual evaluator + native deadlines + LP-node routing — not incremental flags on
this branch. This PR's deliverable is the cutover; the perf goal is re-filed as
future engine work with the entry experiments already done.
