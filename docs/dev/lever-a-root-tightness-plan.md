...
| LR-nvs05 division/ratio entry | **KILL** (2026-07-11) | Probe (`docs/dev/nvs05-division-lever-entry-2026-07-11.md`, `docs/dev/lr0_probe/nvs05_division.py`): keeping C2/C6 (C6's in-sqrt 1e15 simplifies to a modest signomial), exact reciprocal envelopes for every `a/(x·y)`, log-space signomials, + OBBT reach a **root** bound of only **0.78** (best of 32 tangents), ~2% of the root gap — **>50% left = KILL** (§4). **OBBT shrinks NONE of the wide ratio-feeding boxes** (x0,x1∈[0.01,200], x3∈[1,200] unchanged after 10 rounds; the relaxation is too loose to pin a primal var off its bound). The ratio boxes **are** genuinely wide (confirmed), but wide-box + loose-relaxation makes OBBT inert. Gap decomposes 0.674→2.76 (pure-(x2,x3) signomials C5/C6/C7) →5.47 (shear C4 forcing x0,x1 up); closable only by spatial B&B on continuous primals (4001 nodes → 5.208, not certified to tol; 8.8s). **Falsification F17.** nvs05 is research-grade for a near-root cert; its lever is branch-and-reduce throughput, not a root relaxation. |
