...
| P1.3–P1.4 coverage | **P1.4 DONE (2026-07-18)** | **P1.4 fractional/signomial powers**: `MCBox.__pow__` with a non-integer exponent now relaxes `x**a` over a strictly-positive base via `compose_pow_frac` (a>1 convex-inc, 0<a<1 concave-inc, a<0 convex-dec; `clip_inner` boundary-safe subgradients). Sound-or-refuse: a base that can reach `x<=0` (where `x**a` is undefined for non-integer a) yields a no-information bracket. NaN discipline: the envelope is always evaluated on a base clamped `>= eps>0`. Property-tested (bracket, subgradient support incl. box faces, composite `(2x+y+1)**0.5`, no-info on sign-spanning base); the old `test_refuse_noninteger_power` flipped to soundness; end-to-end `x**0.5+y**1.5` model bound ≤ true min. **P1.3 tight monomial hull — EVEN powers DONE (2026-07-18)**: `MCBox.__pow__` for an even integer `n` now uses the exact convex envelope (`compose_even_pow`: `cv = x**n`, secant `cc`) instead of repeated bilinear multiplication — a **material** tightening (measured: `x**2` on `[-2,3]` gives exact `cv(0)=0` vs repeated-mult's `-4`), with valid boundary subgradients via the fixed `clip_into`. Property-tested (bracket, subgradient support incl. corners, exact-cv assertion, composite `(x+y)**2`, `x**4`/`x**6`). **ODD powers DONE (2026-07-18)** too: `compose_odd_pow` builds the tight convex/concave monomial hull (Liberti–Pantelides tangent-line construction) over sign-spanning boxes — genuinely convex/concave (unlike `relax_pow`'s piecewise cv), so the kernel-chain subgradient is valid. The tangent coefficient `c_n` (`t*/|lb|`, e.g. 1/2 for n=3) is a static Newton solve at trace time (homogeneity), and the concave envelope follows by antisymmetry of the odd monomial; boundary subgradients via `clip_inner`. Property-tested (bracket + face subgradients for n∈{3,5,7} over spanning / positive / negative / box-far-negative-chord boxes). So **P1.3 is COMPLETE (even + odd)**. **P1.1b** tight sigmoidal spanning envelope: **terminal at the sound constant-envelope fallback** — unlike the monomial hull (static homogeneous tangent), the sigmoidal tangent point is BOX-dependent (a jit Newton for cv AND cc, per-function symmetry — tanh/atan/sinh odd, sigmoid not — plus tangent-existence edge cases); that is real risk on a correctness-critical path for tightness-only gain, which §0.1 de-prioritizes for reduced-space. The operator is covered (sound); the tight envelope is out of this milestone's scope. |
...
| P2.3 mode wiring | **SOUND, default-OFF (2026-07-10)** | Branch `feat/relax-space-p2.3`: `solver_tuning.relax_space` + env `DISCOPT_RELAX_SPACE` (default `lifted`), reduced bound fed into both the batched and serial node loops, sound-or-refuse fallback. **Default byte-identical: PASS** (11-instance panel unchanged). **Corpus soundness sweep: PASS, incorrect_count=0** across 21 MINLPLib instances (curated small + nvs19/24/st_e36/nvs22 + QPs) cross-checked vs `minlplib.solu` — no false-optimal, no invalid bound, no false-infeasible. **Feasible-point soundness: PASS** (74,015 samples, 0 cuts). **Adversarial suite with the flag ON: 10/10** (incl. nvs22). **nvs22 false-optimal FIXED** — three root causes: (1) unbounded leaves (#582 `_require_finite_box`); (2) the wiring fed the reduced evaluator the LIFTED node box (15 cols incl. ~1e8-bound factorable-reformulation aux vars) instead of the original DOF — fixed by building on `_prereform_model` and slicing node boxes to `_prereform_nvars`; (3) the deep defect — an **INVALID `cc` subgradient of the non-affine division** in nvs22 con2 (`(A·x6)/((x2·x3)·(Σ-of-squares))`), whose Kelley cut excluded the true optimum by ~1.7e5 (numeric witness: cut idx 20 at iterate `[1,1,1,1,42.2,1,1.12,0.064]`). Fixed sound-or-refuse: `_to_mcbox` now **refuses division by a non-affine denominator** (`UnsupportedRelaxation` → status `unsupported` → whole-solve lifted fallback); reciprocal-of-affine is still accepted. Defense-in-depth: an `infeasible` from the in-house simplex is cross-checked against scipy/HiGHS before it is trusted to fathom (never a single mis-solve). nvs22 now certifies 6.0582 (35 nodes, = lifted). nodes/s: reduced ~5× slower/node on st\_e36 where it applies — capability, not speed (§0.1); `auto` still never selects reduced (P2.4). **MERGED via #583 (2026-07-10); wiring is default-OFF/opt-in — no graduation gate (P2.4 KILLED, below).** |
| P2.4 auto policy | **KILLED — entry experiment falsified the win (2026-07-10, task #76)** | ROOT-bound sweep, reduced vs lifted, on a structured MINLPLib sample (~24 instances biased to the hypothesized win class: pooling/bilinear — 16 `pooling_*`/`haverly`/`blend` — plus QP + transcendental controls), each vs the `minlplib.solu` oracle. **Result: reduced-space McCormick's root dual bound is NEVER materially tighter than lifted — it TIES on every valid pooling case and is LOOSER on several** (`pooling_adhya1pq` −856 vs lifted −835; `bental4tp` −550 vs −533; `haverly3tp` −875 vs −869; `st_jcbpaf2` −945 vs −856; `nvs19` −1.03e6, catastrophically loose). Pure reduced root == lifted root on haverly1/2/3pq, bental4pq, rt2pq, foulds2pq, adhya1tp/adhya2pq, st_bpaf1a/b, gbd. **ZERO reduced root wins in any group.** Combined with the post-#583 per-node cost (~8× Kelley tax: nvs19 17 reduced vs 141 lifted nodes / 30 s) and no tightening, there is **no class where reduced pays off** — confirming the plan's binding falsification ("reduced-space tightness ≈ lifted McCormick; the win is structural, not bound tightness"). **VERDICT: reduced-space stays OPT-IN ONLY; `auto` de-scoped, no structural auto-selection predicate exists.** Measurement only — no wiring changed; reduced stays default-OFF. Harness: `scratchpad/p24_root.py`. Re-open only if P3 (CustomCall/hidden-DOF) surfaces a class where the lifted formulation is *unavailable* — then reduced wins by being the only sound option, not by tighter bounds. |
| P2.5 hybrid | **out of milestone scope (2026-07-18)** | Najman-2021 MC↔AVM per-term hybrid. Optional and explicitly gated on its own entry experiment (§4 P2.5); not required for P3 parity. Reduced-space is opt-in and `auto` was killed (P2.4), so a hybrid selector has no consumer today. Left for a future dedicated effort when a motivating dense-integer-QP case is in hand. |
| P3.1 CustomCall relax | **DONE — sound-or-refuse (2026-07-17)** | `_to_mcbox` now relaxes a `CustomCall` by tracing `node.fn` on the argument MCBoxes (`build_reduced_relaxation` + `_build_jit_evaluator` inherit it); a body that traces cleanly yields a genuine `(cv, cc, subgradients)` and the model is globally boundable while its internal intermediates stay hidden. **Soundness (the opaque-body hazard):** a `CustomCall` body has no AST, so the `_is_affine_ast` division guard can't see inside it — a hidden non-affine reciprocal (the nvs22 class) would slip through. Fixed by a new `mcbox.strict_division()` context wrapping the trace: variable-denominator (`MCBox/MCBox`) division REFUSES (`UnsupportedMcboxOp`→`UnsupportedRelaxation`→lifted fallback); constant-denominator division stays sound. Untraceable bodies (raw `jnp` intrinsics on an MCBox, unsupported ops) also refuse → local-NLP, unchanged behavior. Tests: `python/tests/test_customcall_reduced.py` (valid-lower-bound, box-sampled soundness, non-affine-division refusal, untraceable-body refusal, strict-division CM, reduced≤flattened). `.nl` export refusal kept (hidden fn has no `.nl` form). **HARDENED (2026-07-18): continuous CustomCall models now solve GLOBALLY with a certificate via the solver.** `solve_model`'s CustomCall gate (`solver.py`) previously forced the local-NLP path for *every* CustomCall model; it now runs an admission probe (`_custom_call_reduced_admissible` = the one-round reduced-space Kelley probe on the root box) and, for a **continuous, MCBox-relaxable** model, forces `relax_space=reduced` (`_force_reduced_space`) and falls through to the global spatial B&B — branching DOF-only, hidden intermediates hidden. Sound-or-refuse: a non-relaxable body (raw jnp, non-affine hidden division, non-scalar leaf, unbounded box) keeps the local-NLP path exactly as before; **integers + CustomCall still raise** (that is P3.2). The pipeline is safe to fall through: the Rust tree is a pure box/bound manager, and the lifted-McCormick build / Rust `model_to_repr` / FBBT all soft-fail or are repr-gated on an opaque node (verified by pipeline trace); the reduced-space bound replaces the lifted LP per node. **FBBT decision (recorded per plan §5-P3.1):** hidden models **skip Rust FBBT** (the Rust IR has no opaque node → `model_to_repr` raises and is soft-caught); the structure passes (`monotonicity`, `sparsity`, `objective_epigraph`, `uniform_relax`, `interval_ad`) already abstain on `CustomCall`; the `CustomCall` output's interval comes solely from the **MCBox `lo/hi`** propagated during the reduced-space trace (Python interval only). The *lifted* `relaxation_compiler` still raises on `CustomCall` (an opaque node has no aux column) — that is the correct sound-or-refuse for the lifted path; the reduced-space engine is the sound global route and never goes through it. `CustomCall` docstring "consequences" updated. Verified backend-free (this env has no NLP backend): admission logic (`test_customcall_reduced.py` 7 admission cases), routing engages (gate log), and the reduced dual bound is ≤ true opt (no false optimal). The end-to-end certified `m.solve()` (`test_custom_udf.py::test_scalar_custom_solves_globally_certified`, `test_custom_in_constraint`) needs an NLP backend for node incumbents and runs in CI. |
| P3.2 integers | **DONE — sound-or-refuse (2026-07-18)** | The integer guard at the CustomCall gate is now lifted **only for MCBox-relaxable models**: a body that passes the admission probe (`_custom_call_reduced_admissible`) is branched over the integer + continuous DOF with reduced-space Kelley node bounds and certified globally (the Rust tree already does MINLP integer branching; the reduced bound fathoms nodes). A **non**-relaxable body + integers still **raises** (no valid node relaxation → sound-or-refuse). Verified backend-free: integer+admissible flows through the global path without crashing on the integer-specific passes (`binary_multilinear`/`integer_product_reform`/`implied_integer` soft-fail or abstain on `CustomCall`) and the reduced dual bound is ≤ true opt (no false optimal); non-admissible+integer raises at the gate. Tests: `test_customcall_reduced.py` (integer admission, non-admissible+integer raise — both backend-free); `test_custom_udf.py::test_admissible_custom_with_integer_solves_globally` (end-to-end certified, CI/backend); the two prior integer/binary raise tests re-pointed to a non-relaxable body. The end-to-end certified `m.solve()` over integers needs an NLP backend (CI). |
| P3.3 flowsheet gate | **PASS — GO (2026-07-17)** | Reactor-cascade flowsheet, 3 sequential `CustomCall` units (`c_i = c_{i-1}·exp(-a_i T_i)`, hidden intermediates `e_i`, `c_i`), 4 continuous DOF (T1,T2,T3,F0-via-z) + 1 integer (z∈{0,1,2}), objective `-reacted + Σ w_i T_i(c_{i-1}-c_i)`, `reacted ≥ D`. Solved two ways with the **same** engine (`reduced_mccormick_lp_bound`, scipy LP) and the **same** best-first spatial-B&B driver, shared incumbent = true optimum (isolates relaxation/branching from heuristics), 1% optimality gap: **(a) reduced** — CustomCall units, branch DOF-only (4 vars); **(b) flattened** — honest full-space form, every `e_i`/`c_i` an explicit var, branch the whole lifted set (10 vars). **Result: reduced 29 nodes vs flattened 174 nodes = 6.0× smaller** (gate ≥5×), **both certify the same optimum** (reduced_lb −1.5397, flattened_lb −1.5372, opt −1.5116; both valid lower bounds ≤ opt). **VERDICT: GO** — the DOF-tree-size claim holds; P3 proceeds (P3.1 harden, then P3.2 integers). Harness: `scratchpad/p33_flowsheet.py`; permanent soundness test: `test_customcall_reduced.py`. **NB (found + fixed en route):** the entry experiment surfaced a latent invalid-subgradient bug in the intrinsic envelopes at box faces — see the P1.1/P1.2 rows and the falsifications list. Without that fix the flattened root bound was −1.375 > opt (a false bound); the GO measurement was taken only after it was corrected. |
| P4 | **out of milestone scope (2026-07-18)** | Implicit-function / algorithm relaxation (Stuber–Scott–Barton). The plan's own rule: do NOT schedule until P3 ships AND a concrete user model demands it — no such model exists today, and MPI scale-out is an explicit non-goal (§8). Deepest, least-common MAiNGO exclusive; a future dedicated issue + entry experiment when a motivating model appears. |
...
- Reduced-space tightness ≈ lifted McCormick on the dense-QP probes at the root
  ...
  root bounds on QPs. **Now generalized (P2.4 entry experiment, task #76,
  2026-07-10): reduced-space's root bound is ≤ lifted's on the POOLING/BILINEAR
  class too** (16 pooling/haverly/blend instances: ties or reduced-looser, zero
  wins) — so there is NO structural class that motivates `auto` selecting reduced
  by bound tightness. Reduced-space is opt-in capability only; `auto` never
  selects it. The *only* future justification is P3 expressiveness (hidden-DOF
  CustomCall models with no lifted form), not bound quality.
- The FT-storm motivation for reduced space is **gone** (#573 density route);
  §0.1's capability framing is the binding rationale.
- **The intrinsic McCormick subgradients were INVALID on box faces** (found + fixed
  2026-07-17, during the P3.3 entry experiment). `_univariate_kernel` derives the
  subgradient by `jax.grad` through the composition kernel, and the kernels clamp the
  argument with `jnp.clip(cv_g, g_lb, g_ub)`. `jnp.clip` is `min(max(·))`, so at a value
  **on the box boundary** (`cv_g == g_lb`/`g_ub`) `jax.grad` returns the max/min *tie*
  (a 0.5-halved subderivative) instead of the valid inner one-sided slope. Interior
  random sampling (the `_check` test) never lands on a face, so it stayed hidden — but a
  Kelley/LP iterate is a polytope **vertex**, always on faces, so it fired on essentially
  every transcendental reduced-space cut. Numeric witness: `exp` cv subgradient at the
  upper box end was `0.5·exp(ub)`, whose tangent rose above `cv` at interior points; on
  the P3.3 flattened flowsheet this pushed the root *lower* bound to −1.375 **above** the
  true optimum −1.512 (a false bound that would false-fathom the optimum). RESOLUTION
  (value-preserving, so bound-neutral on values): `multivariate_mccormick.clip_inner`
  (grad 1 on the *closed* `[g_lb,g_ub]`) and `clip_into` (grad 1 on the active bound at
  the tie) replace the boundary `jnp.clip` in the monotone-convex/concave, S-shaped,
  and mid-rule kernels plus `mcbox._recip_kernel`. Regression test:
  `test_mcbox.py::test_subgradient_valid_at_box_faces` (corner/face base points for
  exp/log/sqrt/softplus/abs/reciprocal/composite). This affected the whole reduced-space
  path on transcendentals (`DISCOPT_RELAX_SPACE=reduced`), not just P3; the fix is a
  soundness correction independent of the CustomCall work.
...
### Delivered state (milestone close, 2026-07-18)

The P3 CustomCall-global line — the plan's primary justification — is **shipped and
sound**: `dm.custom` models whose opaque body traces through MCBox are solved
**globally with a certificate** (`solve_model` admission gate → forced reduced-space),
branching DOF-only, **integers included** (P3.1, P3.1-harden, P3.2), validated by the
P3.3 flowsheet gate (reduced 29 vs full-space 174 nodes, 6×, same optimum) and a docs
notebook (`docs/notebooks/reduced_space_customcall.md`). Operator coverage: full
arithmetic + bilinear, integer powers via the **tight monomial hull** (even and odd,
P1.3), affine + sign-definite reciprocal division, provably-convex intrinsics, S-shaped
intrinsics (sound; spanning fallback), and **fractional/signomial powers** (P1.4). A
latent box-face subgradient bug across all intrinsic envelopes was found and fixed
(§7 falsifications). Terminal sound-or-refuse boundaries, each with recorded evidence:
**non-affine reciprocal** stays refused (P1.2 lift falsified by fuzz); **S-shaped
spanning** stays at the sound constant envelope (P1.1b — tight box-dependent tangent
envelope is out-of-scope tightness); **P2.5 hybrid** and **P4 implicit/algorithm**
remain unscheduled per their own gates (no consumer / no motivating model today).
Every remaining item is a *sound terminal state* or an explicitly-deferred future
effort — not an open correctness gap.
