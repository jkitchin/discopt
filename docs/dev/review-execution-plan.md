...
| 1 | NN-1, NN-2 | nn | ⬜ — PRs in flight. NN-1=C-25 (scaling ignored in bound propagation → unsound big-M) and NN-2=C-26 (tree big-M invalid for out-of-box thresholds) are `in progress` on the C-board (`nn-module-plan.md` T-N0.2/T-N0.3; C-27/C-28 also in progress). No fix or test on `origin/main` yet — sibling worktrees `fix-nn-c25-c26-soundness`, `fix-nn-c27-c28-readers`. |
