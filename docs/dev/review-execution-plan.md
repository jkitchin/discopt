...
- **NM-1=C-32** (`_jax` McCormick) — `relax_asin`/`relax_acos` inverted
  ...
  above the function) → invalid dual bound. Fix `_relax/mccormick.py` + docstrings;
...
perf/robustness); the **P2/P3 remainder** across modules. (The **L2/L3/L4/L7**
latex tail is now ✅ — PR `fix-latex-tail-l2-l3-l4-l7`, #413. The **M4/M5/M6/M7/M8**
modeling P1/P2 guardrails are now ✅ — branch `fix-modeling-tail-m4-m8`,
`test_modeling_tail_m4_m8.py`; markers in `modeling-module-review.md`.)
...
| 1 | NM-2 | _numpy McCormick (deleted) | ✅ — deleted the dead, compiled-but-unused `discopt._numpy` backend (behavior-preserving: params accepted but never consumed; POUNCE builds the bound from the JAX relaxation alone). Removed `_numpy/` pkg, `solver.py` compile block + 2 call sites, dead `obj/con_relax_fns_numpy` params in `_relax/mccormick_nlp.py`, numpy `_BACKENDS` id in C19/C23/C32 envelope tests, stale mypy override; #413 |
...
| — | **gdp: GDP-1 (P0), GDP-2** | gdp (`_relax/gdp_reformulate.py`) | ✅ — **focused GDP soundness review (#413 `[ ] gdp?`), `docs/dev/gdp-module-review.md`.** **GDP-1** (P0 default-path false certificate): `_compute_big_m` treated any variable bound ≥ `_INF_THRESH=1e15` as "infinite" and substituted `_DEFAULT_BIG_M=1e4`, shrinking the *valid* large-but-finite default bound (±~1e20) to a too-small M → the inactive disjunct's relaxed constraint stayed binding and cut the whole **active** disjunct. Repro `min (x−25000)² s.t. (x≤2)∨(x≥20000)` with default bounds → `status="infeasible"` (true opt x=25000, obj=0). Fix: a finite bound (even huge) is a valid M — use it; a *truly* infinite bound has no valid finite M → **refuse loudly** (never silently clamp). Same defect+fix in the SOS linking big-M. **GDP-2**: `_compute_big_m_lp` built the LP `bounds` from the *mutated* model (with selector aux) → longer than the precomputed `c`/`A` columns → exact-LP backend panic on every mbigm/auto disjunction; fix aligns `bounds` to `n_vars`. Differential harness 0-cut/0-admit post-fix; exhaustive truth-table check of all logic encodings SOUND; hull/perspective, SOS-cardinality, LOA convexity-gating + C-35, and the advisor all verified sound. Tests: `test_gdp1_gdp2_bigm_soundness.py` (5, fail-before/pass-after) + rewritten `test_gdp.py::TestComputeBigM`. **Cert-baseline NEUTRAL — EXACT**: `reformulate_gdp` no-ops on non-GDP models, and all 41 baseline `.nl` instances carry no GDP constructs → `_compute_big_m` never reached → node_count/objective bit-identical, `incorrect_count == 0`. #413 |
| 2 | M3 | modeling | ✅ `test_m3_cross_model_ownership` — validate() rejects index-incompatible cross-model leaves |
| 3 | MO1–MO10 ✅ (MO10 confirmed+deferred) | mo | ✅ COMPLETE — MO1–MO4 in prior PRs (`test_mo_augmecon2.py`, `test_mo_pareto.py::TestFilteredDedup`, `test_mo_indicators.py::TestCommonReference`); this PR closes the tail: **MO5** (compile each objective once per sweep via `ObjectiveEvaluator` — bound-neutral, fronts bit-identical before/after, recompiles 26→0 on an 11-pt bi-obj sweep) ✅, **MO6** (honor supplied `ideal`, skip anchor solves) ✅, **MO7** (`total_time_limit` + large-grid warning on all five sweeps) ✅, **MO8** (identity-based constraint cleanup) ✅, **MO9** (drop dead `_payoff_matrix` `senses_list`) ✅ — `test_mo_tail_mo5_mo10.py`; **MO10** (AMP drops parameter-carrying constraints) ◻︎ CONFIRMED live, SOUND-but-weak, deferred to the Phase-3 solver-integration round (bound-changing, needs differential-bound protocol). cert-baseline NEUTRAL (mo off the B&B solve path). |
...
| 3 | E1–E3 ✅ · L1/L5/L6/L8 ✅ · L2/L3/L4/L7 ✅ · M4/M5/M6/M7/M8 ✅ | modeling/examples/latex | ✅ — **E1–E3** RESOLVED (`modeling-module-review.md` STATUS; `test_examples_gallery.py` on `origin/main`: Haverly pooling certifies 400, logical-constraints example builds, reactor feasible). **L1/L5/L6/L8** RESOLVED (`modeling-module-review.md` STATUS; `test_display_cluster.py` on `origin/main`). **L2/L3/L4/L7** ✅ (#413, `fix-latex-tail-l2-l3-l4-l7`): all four confirmed (fails-before on `origin/main`) then fixed in `modeling/latex.py`. **L2** — `model_to_latex`/`model_to_html` now route through the X-1 primitive (`export._common.iter_builder_linear_rows`) so builder-resident fast-path rows render as real constraints and the header count includes them (was "0 constraints"); L1 GDP/non-arithmetic rows still rendered from the full `_constraints` list. **L3** — `expr_to_latex` handles `SumOverExpression` (proper `\sum(...)` of expanded terms, ≤8 then `+\cdots`, was raw `Σ[N terms]`) and `Parameter` (escaped `\_` symbol, was `param(price_A)`). **L4** — `parent_prec` threaded through UnaryOp/`SumExpression`/`SumOverExpression`/MatMul via a `_paren` helper so they parenthesise under a tighter parent (`(-x)**2` → `\left(-x\right)^{2}`, sum/matmul `**2` wrapped; was `-x^{2}` etc.). **L7 CONFIRMED real** — split the conflated `_escape_text` into `_latex_text` (math-mode: escapes `% _ # & $ { } \ ~ ^`, wraps in `\text{}`, no HTML entities) and `_escape_html` (HTML chrome only); math fallbacks no longer inject `&amp;` and now escape `_`/`%`/`#`. Tests fail-before/pass-after: `test_display_cluster.py::test_l{2,3,4,7}_*` (9 new). Cert-baseline NEUTRAL (only `latex.py` display code touched). **M4/M5/M6/M7/M8** ✅ (branch `fix-modeling-tail-m4-m8`, `test_modeling_tail_m4_m8.py`, 21 tests): M4 identity hashing + `Constraint.__bool__` raises (DSL `==`-overload kept); M5 copy-on-name (identity-preserving first add, copy on re-add so an earlier row is never renamed) + duplicate-constraint-name rejection in `validate()`; M6 `solve()` kwarg allowlist (`solve_model_accepted_kwargs()`, `TypeError` + near-match; 3 latent swallowed no-ops fixed at source, not allow-listed); M7 persistent `_names` set (O(n²)→O(1): measured 12.8/24.1/41.8 → flat ~2.6 µs/var at n=1k/2k/4k); M8 conservative build-time broadcast check (raises only when both shapes known-incompatible, never on unknown). Cert-baseline EXACT-neutral (20 `.nl` instances: status/obj/nodes bit-identical); smoke 540 passed, adversarial 10 passed; ruff/mypy clean. #413 |
...
- **Solver core proper** (`solver.py` B&B loop, `_relax/` relaxation layer,
