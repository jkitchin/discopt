...
| C-43 | P0 | simplex/Farkas certificate | `farkas_ray_certifies_cols` built its Neumaier–Shcherbina margin from the sizes of the *results* (`1e-9·(1 + \|bᵀy\| + Σ\|boxmax\|)`), which bound the rounding of none of the accumulations that produce them — so a `bᵀy = 3e-8` assembled from terms summing to `600` (5e-11 relative, pure cancellation) cleared it and the engine emitted `LpStatus::Infeasible`, a **fathoming proof**, for an LP that HiGHS, three perturbed pivot paths of our own engine and an elastic feasibility LP all solve feasible (= issue #1017). Defect in the C-38 verification itself; DEFAULT path | **fixed** (`docs/dev/1017-farkas-accumulation-margin-2026-08-13.md`): the margin now carries `γ_k·Σ\|terms\|` for the row dot, per-column interval enclosure of `Aᵀy`, and rounded-up implied slack bounds; kept at `max(legacy, rigorous)` so it can only withdraw a certificate. 16-instance Farkas-active panel: 335 certifications, node counts/bounds identical, 0 withdrawn |
