...
| EP3 patch-table node path | **REVERTED — unsound tightness regression (bound-changing, not neutral)** | EP3's `UniformPatchTable` regenerated the node relaxation through the EP1-cached build then had the per-node solve **skip the separation chain**, substituting the inherited *root* cut pool. That substitution is NOT bound-neutral: per-node separation re-run at a *tightened* child box can be strictly stronger than the root pool, so skipping it yields a weaker (still valid, ≤ optimum) dual bound. The per-instance EP3 gate passed only because nvs09/ex1226 happened to be node-neutral (root pool already sufficient). The maintainer's `global50` panel (PR #636, 2026-07-14) caught the real cost: **st_e04 153→18873 nodes, tspn05 39→3303, nvs19 — 3 lost proofs**, the dual bound stalling below the cold-rebuild bound. Bisected to `2c21348`. **Diagnosis (in-container): the per-node cost IS the separation (nvs09 cold ≈64 s, dominated by the `_separate_convex` Kelley loop + multilinear facet LPs; the build itself is ~12 ms after EP1). EP3's "win" was skipped necessary work, not a cheaper node — a real solver (BARON/SCIP) never trades per-node tightness for speed.** So EP3 is reverted in full (`UniformPatchTable` class + engagement + its flag + bespoke tests removed; the 3 EP3-only files restored to the pre-EP3 `0935555` state; the 3 incremental tests returned to `_CUTOVER_DEFERRED_TESTS` as before). Default node path = the fully-separating cold build again → **tspn05 back to optimal / 39 nodes / 191.255, st_e04/nvs19 proofs restored**. EP1 + EP5 (byte-neutral, always-on caching — the genuine, correct wins) are untouched. A sound "cheaper node" needs either genuinely cheaper separation (EP4a/EP4b measured negligible) or fewer nodes (EP6 branch-and-reduce) — not skipping separation. | (revert) |
