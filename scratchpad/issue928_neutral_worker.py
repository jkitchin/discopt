"""#928 neutrality-check worker that runs on EITHER tree.

CLAUDE.md §8: the marker is asserted in both directions — ``expect_marker=1``
requires ``_dual_start_slack_basis`` (the #928 slack-dual start helper) to be
present in ``discopt.solvers.milp_simplex``, ``0`` requires it absent — so a
...
present = hasattr(_ms, "_dual_start_slack_basis")
assert present == (expect == "1"), (
    f"marker mismatch: _dual_start_slack_basis present={present}, expected={expect == '1'}"
)
