...
First pass (probe_isolate.py), row filter pinned OFF, time_limit=25:
...
for rep in range(3):
    for arm in ("0", "1"):
        ...
        os.environ["DISCOPT_NODE_NUMERICAL_DUAL_BOUND"] = arm
        ...
        print(
            f"rep={rep} 517={arm} status={r.status} bound={r.bound} sound={sound}",
            flush=True,
        )
