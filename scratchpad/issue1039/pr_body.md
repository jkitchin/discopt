...
`_relax/gdp_reformulate.py` silently dropped its exactly-one row. `_is_linear`
had no `SumOverExpression` case, and `_extract_body_coeffs` skipped that node
type outright, so a GDP whose exactly-one constraint is built as `Σ[N terms]`
reformulated without it — LOA then returned `unknown`. The module-level import
of `SumOverExpression` was also missing (the only one was function-local), so the
first patch raised `NameError` and said so loudly. 354 GDP tests pass.
...
real defects (the super-optimal division objective, the root-setup overrun, the
sonet23v4 contradiction, and nvs19's budget non-monotonicity), and per §3 a
workaround ships with an issue tracking the real fix. Drafts are in
`scratchpad/issue1039/`; say the word and I will file them and link them from the
xfail reasons.
