...
for p in sorted(glob.glob("docs/notebooks/*.ipynb")):
    ...
    never = [c for c in code if c.get("execution_count") is None
    ...
    for c in code:
        for o in c.get("outputs", []):
            ...
            if "\x1b[" in t: flags.append("ansi")
            if " INFO " in t: flags.append("rust-INFO")
            if "timing-bucket-unknown" in t: flags.append("timing-bucket")
            if SKIP.search(t): flags.append("skip-word")
