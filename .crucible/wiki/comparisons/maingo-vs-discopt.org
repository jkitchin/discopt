...
- **McCormick relaxations as the bounding primitive.** Both derive convex/concave
  ...
  =python/discopt/_relax/multivariate_mccormick.py=; MAiNGO implements it in MC++.
...
=python/discopt/_relax/mccormick_lp.py= — whose own docstring reads "bilinears lifted
...
| Relaxation math | MC++ (McCormick + subgradients) | JAX DAG compiler (=_relax/=), autodiff |
