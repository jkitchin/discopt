...
\textbf{Specialized envelopes versus compositional relaxations.} The envelope functions for power, exponential, and logarithmic operations provide exact convex/concave envelopes, avoiding the composition error inherent in McCormick's recursive approach. The relaxation compiler automatically detects when a specialized envelope is applicable and routes accordingly. Extending the envelope library to cover trigonometric functions (sin, cos on bounded intervals) and general signomial terms ($\prod x_i^{a_i}$) is straightforward and would improve relaxation quality on engineering design problems.
