...
so termination at the first $k$ with closed gap is a certified $\varepsilon$-optimal solution. The MILP-relaxation construction is shared with the convex-hull / Big-M formulation work used elsewhere in discopt and is implemented in =discopt._jax.milp_relaxation=. The MILP itself is solved by HiGHS through the existing solver dispatch (Section ref:sec-nlp-backends), and the NLP subproblem by any of the three NLP backends (default =pounce=; cyipopt is selected automatically when available).
