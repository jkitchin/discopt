...
| Feature                  | BARON | SCIP    | Bonmin | cvxpylayers  | discopt |
|--------------------------+-------+---------+--------+--------------+---------|
| Nonconvex MINLP          | Yes   | Yes     | Yes    | No           | Yes     |
| Differentiable solve     | No    | No      | No     | Yes (convex) | Yes     |
| Batched node evaluation  | No    | No      | No     | N/A          | Yes     |
| Memory-safe solver       | No    | No      | No     | N/A          | Yes     |
| Zero C/Fortran deps      | No    | No      | No     | Yes          | Yes     |
| DFL through MINLP        | No    | No      | No     | No           | Yes     |
| Compositional relaxation | Yes   | Partial | No     | N/A          | Yes     |
...
The API supports continuous, binary, and integer variables; parameters (for sensitivity analysis and decision-focused learning); generalized disjunctive programming (GDP) constraints with automatic big-M reformulation; and standard mathematical functions (=exp=, =log=, =sin=, =cos=, =sqrt=, =abs=, and matrix operations).
