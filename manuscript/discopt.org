...
The API supports continuous, binary, and integer variables; parameters (for sensitivity analysis and decision-focused learning); generalized disjunctive programming (GDP) constraints with big-M, multiple big-M (MBM), and convex hull reformulations; =BooleanVar= with propositional logic operators (=&=, =|=, =~=, =.implies()=, =.equivalent_to()=) and CNF-to-linear conversion; nested disjunctions for hierarchical process design; and standard mathematical functions (=exp=, =log=, =sin=, =cos=, =sqrt=, =abs=, =tanh=, =sigmoid=, =softplus=, and matrix operations).
...
*Future work.* Key directions include: (1) GPU-accelerated batch IPM via JAX's XLA backend when GPU support matures; (2) extending ripopt with sparse linear algebra to compete with Ipopt on large problems; (3) deeper integration of learned relaxations with online training during the B&B search; (4) exploiting the differentiable solver for bilevel optimization and meta-learning of solver parameters; (5) extending the DAE collocation module to higher-index DAEs, adaptive mesh refinement, and multi-stage optimal control with path constraints; and (6) extending the neural network embedding to convolutional layers, residual connections, gradient-boosted tree models, and LP-based bound tightening (OBBT) for tighter big-M constants.
