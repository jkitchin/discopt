...
so termination at the first $k$ with closed gap is a certified $\varepsilon$-optimal solution. The MILP-relaxation construction is shared with the convex-hull / Big-M formulation work used elsewhere in discopt and is implemented in =discopt._relax.milp_relaxation=. The MILP itself is solved by the self-hosted pure-Rust simplex engine (Section ref:sec-lp-engine), and the NLP subproblem by any of the three NLP backends (default =pounce=; cyipopt is selected automatically when available).
