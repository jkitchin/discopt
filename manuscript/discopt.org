...
#+NAME: tab-minlplib
#+CAPTION: MINLPLib smoke test results (9 instances, 60s time limit, v0.8.0, Apple M4 Pro). Best known objectives from MINLPLib. Reproduced by the manuscript smoke-test notebook.
| Instance | $n$ | $m$ | Status   | $f^*$      | Best Known | Time (s) | Nodes  |
|----------+-----+-----+----------+------------+------------+----------+--------|
| chance   |   4 |   3 | optimal  | 29.894     | 29.894     |    0.033 |      0 |
| dispatch |   4 |   2 | optimal  | 3155.288   | 3155.288   |    0.198 |     23 |
| ex1221   |   5 |   5 | optimal  | 7.667      | 7.667      |    0.189 |      5 |
| ex1226   |   5 |   5 | optimal  | -17.000    | -17.000    |    0.331 |      5 |
| gear     |   4 |   0 | optimal  | 0.000      | 0.000      |    0.147 |     31 |
| nvs01    |   3 |   3 | optimal  | 12.470     | 12.470     |    0.289 |      3 |
| nvs03    |   2 |   2 | optimal  | 16.000     | 16.000     |    0.025 |      3 |
| nvs04    |   2 |   0 | optimal  | 0.720      | 0.720      |    0.047 |      5 |
| nvs06    |   2 |   0 | optimal  | 1.770      | 1.770      |    0.168 |      5 |
...
*Time breakdown.* An earlier revision of this paper reported that JAX numerical computation dominated at 92--99.9% of wall time, with Rust tree management at 0.05--7.3% and Python orchestration at 0.02--0.6%. That measurement is *superseded and retracted*: it described a build in which every nonlinear solve routed its derivatives through the JAX evaluator, and the default path no longer does. Re-measured on the v0.8.0 tree over ten =.nl= instances (three repeats each, interleaved, Apple M4 Pro), JAX accounts for $0.00\%$ of wall time on every one --- =jax= never enters =sys.modules= during a default solve. The remainder divides between the Rust layer ($1.5$--$98.5\%$) and Python orchestration ($1.0$--$84.0\%$), and that split is governed by how much search the instance actually needs rather than by a language boundary: on a short solve the fixed cost of import, =.nl= parsing, and model construction dominates (=alan=, 21 nodes: $84.0\%$ Python, $8.0\%$ Rust), while on the one instance in the set that does substantial search the Rust engine dominates (=tanksize=, 17,095 nodes: $98.5\%$ Rust, $1.0\%$ Python). The claim that the multi-language architecture introduces negligible /interop/ overhead is unchanged and separately measured above; what is corrected is which layer the work is in.

*Where JAX is, and is not.* The derivative evaluator on the default path is an AD tape built by POUNCE, which supplies $f$, $\nabla f$, $g$, $J$, and the Lagrangian Hessian with no JAX at all. It graduated to default-ON under the flag discipline of Section ref:sec-soundness after agreeing with the JAX evaluator over 66 corpus instances to $5.5 \times 10^{-16}$ ($f$), $3.8 \times 10^{-16}$ ($\nabla f$), $3.1 \times 10^{-15}$ ($J$), and $7.8 \times 10^{-13}$ (Lagrangian Hessian) --- against pre-registered bars of $10^{-10}$ on the gradient and Jacobian and $10^{-8}$ on the Hessian. The JAX evaluator remains in the tree behind =DISCOPT_NLP_EVAL=jax=. Measuring that opt-out arm on the same ten instances puts the JAX share at $3.4$--$12.9\%$: an order of magnitude below the retracted figure /even with JAX switched on/, because the dominant cost was never the derivative evaluation. Node counts are identical between the two arms on nine of the ten instances (=tanksize= differs, 17,095 against 17,203, which is why swapping AD engines was treated as bound-changing rather than as a refactor), and the tape arm is faster on every instance that loads JAX at all.
