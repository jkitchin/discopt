...
*BARON head-to-head.* On a curated set of 50 MINLPLib instances spanning convex and nonconvex MIQP, nonconvex MINLP, and nonconvex NLP, we compare discopt against BARON cite:&Tawarmalani2005 run identically through GAMS, under a matched $120$~s per-instance budget and $10^{-4}$ relative gap tolerance. discopt matches the proven global optimum of /every/ instance it solves, with *zero correctness violations*, certifying $47$ of the $49$ parseable instances (one instance is excluded for a binary-format =.nl= our text parser does not read). The two remaining are honest time-limit misses --- for one of which BARON also returns no certified solution --- not incorrect results. discopt is competitive with BARON on the convex and small instances and slower on hard nonconvex ratio-of-products structure, where the loose factorable relaxation, rather than the branching search, is the bottleneck (Section ref:sec-discussion); closing that node-count gap on the dense-integer-product class is the principal open performance item.
