...
1. *Differentiable MINLP* via two mechanisms: the envelope theorem (Level 1) for fast objective sensitivity, and implicit differentiation of the KKT system (Level 3) for full primal--dual sensitivity matrices, enabling decision-focused learning through nonconvex solvers (\S 6).
2. *Batched node evaluation* using =jax.vmap= over NLP relaxation solves at B&B nodes, with warm-starting from parent solutions (\S 4.1).
3. *Pure-JAX interior point method* with Mehrotra predictor-corrector cite:&Mehrotra1992, Cholesky-based inertia correction, Ipopt-style filter line search cite:&Wachter2006, and carry-based =jax.lax.while_loop= for JIT cache reuse (\S 4.1).
4. *ripopt*: a source-level C++ \to Rust translation of Ipopt's core IPM algorithm, producing a standalone memory-safe NLP solver with a clean =NlpProblem= trait interface (\S 4.2).
5. *Compositional relaxation framework* with 21 McCormick relaxation functions cite:&McCormick1976;&Tsoukalas2014, \alpha BB underestimators cite:&Androulakis1995;&Adjiman1998, piecewise McCormick with adaptive partitioning cite:&Castro2015 achieving 91--94% gap reduction, and ICNN-learned relaxations cite:&Amos2017icnn (\S 5).
6. *Neural network embedding* that formulates trained feedforward networks (ReLU, sigmoid, tanh, softplus) as MINLP constraints via big-M, full-space, and reduced-space strategies, with interval arithmetic bound propagation and ONNX model import cite:&Ceccon2022;&Anderson2020 (\S 3.4).

The remainder of this paper is organized as follows. Section 2 reviews background and related work. Section 3 describes the discopt architecture, including the modeling API, DAE collocation, neural network embedding, and compiler infrastructure. Section 4 presents the three NLP solver backends. Section 5 details the convex relaxation framework. Section 6 covers differentiable optimization. Section 7 reports computational experiments. Section 8 discusses limitations and future work, and Section 9 concludes.
...
The =discopt.dae= module extends the modeling API to dynamic optimization problems by transcribing ODE/DAE systems into algebraic NLP constraints using orthogonal collocation on finite elements cite:&Biegler2010,Cuthrell1987. A =DAEBuilder= class provides a high-level interface: users declare state, algebraic, and control variables, supply an ODE/DAE right-hand side as a Python callable that builds discopt expressions, and call =discretize()= to generate all collocation and continuity constraints automatically.

The module supports both Radau IIA and Gauss-Legendre collocation schemes with 1--5 collocation points per element. First-order ODEs, second-order ODEs (with automatic state augmentation for velocity coupling), and index-1 DAEs (algebraic constraints enforced at collocation points) are supported. Piecewise-constant control variables and integral objectives via quadrature are included for optimal control problems. A finite-difference backend (=FDBuilder=) provides backward, forward, and central Euler stencils as a simpler alternative.

This transcription approach requires no changes to the existing DAG compiler, JAX backend, or solver infrastructure---the collocation constraints are standard algebraic expressions built from existing operators. For a 30-element, 3-point Radau discretization of a single-state ODE, the exponential decay solution matches =exp(-t)= to within $10^{-4}$ absolute error.
...
#+BEGIN_EXAMPLE
  min_x  f(x, p)     s.t. g_i(x, p) ≤ 0  for all p ∈ U
#+END_EXAMPLE
...
- *Box* ($L_\infty$ ball) $\mathcal{U} = \{p : |p_j - \bar{p}_j| \le \delta_j\}$: the worst-case maximisation of an affine function $c^\top p$ over the box is $\bar{c}^\top p + \sum_j \delta_j |c_j|$ --- a separable 1-norm penalty.  When $p$ appears only as an additive RHS term, this collapses to a single bound substitution $p \leftarrow \bar{p} \pm \delta$.
...
*** Implementation: Sign-Tracking Substitution

The reformulation traverses the expression DAG while tracking the *effective sign* of each uncertain parameter at each node.  At a subtraction node $a - p$, the parameter $p$ acquires sign $-1$; at a negation node $-p$, the sign flips.  Multiplication by a non-negative constant preserves the sign; multiplication by a negative constant flips it.  This sign determines whether the worst-case value is the upper or lower component bound:

#+BEGIN_EXAMPLE
  sign > 0 and maximize ⟹ use upper bound  (p̄ + δ)
  sign < 0 and maximize ⟹ use lower bound  (p̄ − δ)
#+END_EXAMPLE

This is correct for all affine occurrences of uncertain parameters.  Bilinear terms ($p \cdot x$ where both are free) require additional auxiliary variables and are flagged with a diagnostic message.
...
For the additive/affine uncertainty case, the sign-tracking substitution produces an exact robust counterpart: the reformulated constraints hold for all $p \in \mathcal{U}$ if and only if they hold after substituting the worst-case constants.  This follows directly from the LP duality arguments in cite:&BenTal1999.  The implementation verifies this via structural tests that confirm (1) no uncertain parameters remain in the reformulated DAG, and (2) the embedded worst-case constants match the analytically derived values.
...
# Stage 1: substitute recourse variable y with y₀ + Y₀·(d - d̄)
adr = AffineDecisionRule(y, uncertain_params=d)
adr.apply()          # retires y; adds y₀, Y₀ to model._variables

# Stage 2: static robust counterpart handles remaining ξ terms
rc = RobustCounterpart(m, BoxUncertaintySet(d, delta=20.0))
rc.formulate()       # fully deterministic; optimise over (x, y₀, Y₀)
#+END_SRC

The DAG substitution walks every constraint and objective node, replacing Variable nodes that match the recourse variable with the affine expression via identity comparison ($O(|V| + |E|)$ in the DAG size).  After substitution the recourse variable is retired from the model's variable list and remaining variables are renumbered.  A call to =AffineDecisionRule.apply()= followed by =RobustCounterpart.formulate()= leaves the model fully deterministic with (1) no recourse variables, (2) no uncertain parameters, and (3) new intercept and policy-column variables in their place.
